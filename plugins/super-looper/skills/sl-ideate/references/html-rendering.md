...
consumers that read HTML today (`sl-work`, human readers) do so directly;
the agent-consumability rules below make that work. `sl-doc-review` is
*not* currently an HTML consumer — its mutation mechanics are markdown-only,
so the sl-plan handoff gates the 5.3.8 doc-review pass to `OUTPUT_FORMAT=md`
...
- **Source / composition signal.** A visible footer at the bottom of
  ...
  `<footer class="composition-signal">Composed 2026-05-17T14:23Z by sl-plan from <code>docs/brainstorms/...-requirements.md</code></footer>`.
...
Downstream agents that read HTML today (`sl-work`, a skill re-reading its
...
script-style parse. `sl-doc-review` is not a current HTML consumer (see
opening note).
