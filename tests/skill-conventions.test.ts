 ...
 *        sl-demo-reel's `![Before](url-1)` PR template), and fences are where
 ...
 *        anti-pattern examples (sl-plan, sl-sessions, sl-ideate).
 *
 ...
 *    HTML comments inside sl-compound's and sl-compound-refresh's asset
 ...
 *    script invocations (sl-clean-gone-branches' `bash scripts/clean-gone`),
 ...
 *    must degrade gracefully. Mechanically graceful forms pass
 ...
 *    Credential-style env vars (e.g., GITHUB_TOKEN) are out of scope —
...
const INSTALLED_PLUGIN_PATH_EXEMPTIONS = new Map<string, string>([
  [
    "plugins/super-looper/skills/sl-update/SKILL.md",
    "sl-update is explicitly Claude-Code-only; its purpose is reasoning about the marketplace cache layout it READS (never edits).",
  ],
  [
    "plugins/super-looper/skills/sl-report-bug/SKILL.md",
    "sl-report-bug reads ~/.claude/plugins/installed_plugins.json to capture the installed plugin version for bug reports — runtime machine-state read, not a skill-content reference.",
  ],
])
...
type SkillDir = {
  /** repo-root-relative skill dir, e.g. plugins/super-looper/skills/sl-plan */
  relPath: string
  absPath: string
}
 ...
 * sl-code-review's "Included References"). These are load-bearing — a deleted
...
const PLATFORM_VAR_REGEX = /\$\{?(CLAUDE_[A-Z][A-Z0-9_]*)/g
...
const PLATFORM_VAR_CAPTURE = `\\$\\{?(CLAUDE_[A-Z][A-Z0-9_]*)\\b`
...
const PLATFORM_VAR_ACKNOWLEDGED = new Map<string, string>([
  [
    "plugins/super-looper/skills/sl-setup/SKILL.md#CLAUDE_PLUGIN_ROOT",
    "AGENTS.md pre-resolution pattern: the prose under the pre-resolved line says exactly what to do when it is empty or an unresolved literal (treat as non-Claude-Code; omit /sl-update references).",
  ],
  [
...
describe("extractLocalReferenceCandidates", () => {
  ...
  test("includes fenced script invocations: an existing script passes, a missing one is caught", () => {
    ...
    const skillRoot = path.join(PLUGINS_ROOT, "super-looper", "skills", "sl-clean-gone-branches")
  ...
  test("variable-prefixed invocations are extracted and anchor at the skill root", () => {
    ...
    // demonstrated against a real bundled script (sl-update's).
    const sample =
      '```bash\nbash "${CLAUDE_SKILL_DIR:-.}/scripts/upstream-version.sh"\n```'
    expect(extractLocalReferenceCandidates(sample).map((c) => c.value)).toEqual([
      "scripts/upstream-version.sh",
    ])
    const skillRoot = path.join(PLUGINS_ROOT, "super-looper", "skills", "sl-update")
...
describe("resolveInsideSkill", () => {
  ...
  test("rejects ../ traversal to a sibling skill even when the target exists there", () => {
    const sibling = "../sl-plan/references/plan-handoff.md"
  ...
  test("rejects obfuscated in-prefix traversal (references/../../other-skill/...)", () => {
    expect(
      resolveInsideSkill(skillRoot, skillRoot, "references/../../sl-plan/references/plan-handoff.md"),
    ).toBeNull()
  })
...
describe("findPlatformVarOccurrences / isGracefulPlatformVarUse", () => {
  ...
  test("treats ${VAR:-default} as graceful for ordinary platform vars", () => {
    const occurrences = findPlatformVarOccurrences('echo "${CLAUDE_SESSION_ID:-0}"')
    expect(occurrences).toEqual([{ lineNumber: 1, variable: "CLAUDE_SESSION_ID", graceful: true }])
  })
  ...
  test("treats [ -n \"$VAR\" ] existence guards as graceful", () => {
    const occurrences = findPlatformVarOccurrences('if [ -n "$CLAUDE_SESSION_ID" ] || [ -n "$CLAUDE_PROJECT_DIR" ]; then')
    expect(occurrences.map((o) => o.graceful)).toEqual([true, true])
  })

  test("ignores credential-style env vars like GITHUB_TOKEN", () => {
    expect(findPlatformVarOccurrences("export GITHUB_TOKEN=$GITHUB_TOKEN")).toEqual([])
  })
})
...
describe("findPlatformVarViolations", () => {
  test("reports every non-graceful occurrence (acknowledgment happens via the registry, not the scanner)", () => {
    ...
    // and the real sl-setup use of this pattern is acknowledged in
  ...
  test("graceful uses are not reported", () => {
    expect(findPlatformVarViolations('echo "${CLAUDE_SESSION_ID:-0}"')).toEqual([])
  })
