...
content-search: pattern="module:.*(super-looper|skill-design)" path=docs/solutions/ files_only=true case_insensitive=true
...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with `/sl-compound` after it lands — the absence is itself useful signal.
...
- `/sl-plan` — to inform planning with institutional knowledge and add depth during confidence checking
- `/sl-code-review`, `/sl-optimize`, `/sl-ideate` — to surface prior learnings relevant to the change, optimization target, or ideation topic
