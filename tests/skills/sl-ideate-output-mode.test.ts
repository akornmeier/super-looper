...
const SKILL_DIR = path.join(
  process.cwd(),
  "plugins/super-looper/skills/sl-ideate",
)
...
// sl-ideate gains the same exclusive output-mode machinery as sl-plan and
// sl-brainstorm, but with the default INVERTED to `html` (config key
...
function phase00Region(): string {
  ...
  expect(start, "sl-ideate SKILL.md is missing the Phase 0.0 Output Mode section.").toBeGreaterThan(-1)
  const end = SKILL_BODY.indexOf("#### 0.1", start)
  return SKILL_BODY.slice(start, end > start ? end : start + 4500)
}

describe("sl-ideate output mode (html default)", () => {
  test("argument-hint advertises output:md (the override of the html default)", () => {
    ...
    expect(
      ...
      `sl-ideate argument-hint must mention 'output:md' so humans discover the escape from the html default. Current value: ${JSON.stringify(hint)}`,
    ).toBe(true)
  })
  ...
  test("Phase 0.0 defaults to html, not md", () => {
    ...
    expect(
      ...
      "Phase 0.0 must establish `html` as the default OUTPUT_FORMAT for sl-ideate.",
    ).toBe(true)
  })
  ...
  test("Phase 0.0 describes the resolution mechanics", () => {
    ...
    expect(
      ...
      "Phase 0.0 must name the `ideate_output` config key (the sl-ideate parallel to plan_output / brainstorm_output).",
    ).toBe(true)
    expect(
  ...
  test("ideate-to-brainstorm handoff does NOT auto-propagate output:", () => {
    ...
    expect(
      ...
      "Phase 0.0 must state that output: does not auto-propagate to sl-brainstorm on handoff (sl-brainstorm re-resolves its own brainstorm_output).",
    ).toBe(true)
  })
  ...
  test("Phase 4 auto-writes the deliverable (not opt-in) and shows a concise summary", () => {
    ...
    expect(
      /CE temp area|\/tmp\/super-looper|never the user's CWD|not.*current working directory/i.test(POST_IDEATION_BODY),
  ...
  test("Phase 5 menu offers brainstorm-one and iterate-one, and seeds brainstorm with substance not the whole file", () => {
    ...
    expect(
      ...
      "§5.2 must seed sl-brainstorm with the idea's substance, not the whole file.",
    ).toBe(true)
  })
  ...
  test("section contract and rendering refs exist at parallel paths", () => {
    ...
    // (byte-identical to sl-plan) and their invariants by html-output-invariants.test.ts;
    // here we only confirm sl-ideate ships both copies.
