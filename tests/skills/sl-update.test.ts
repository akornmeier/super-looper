...
describe("sl-update SKILL.md", () => {
  // Regression guard for https://github.com/akornmeier/super-looper/issues/556.
  //
  // `CLAUDE_PLUGIN_ROOT` points at the currently-loaded plugin version directory
  // (e.g. `~/.claude/plugins/cache/<marketplace>/super-looper/<version>`),
  // NOT the plugins cache root. Appending `/cache/<anything>/super-looper/`
  ...
  test("does not append a /cache/<marketplace>/ suffix onto CLAUDE_PLUGIN_ROOT", () => {
    ...
    expect(
      ...
      "sl-update/SKILL.md reintroduced the ${CLAUDE_PLUGIN_ROOT}/cache/... antipattern — derive the cache dir from dirname \"${CLAUDE_PLUGIN_ROOT}\" instead.",
    ).toBe(false)
  })
  ...
  test("does not use `!` pre-resolution to invoke bundled scripts", () => {
    ...
    expect(
      preResolutions,
      `sl-update/SKILL.md must not use '!\`bash <path>.sh\`' pre-resolution — it hits Claude Code's load-time permission check, which does not honor 'defaultMode: bypassPermissions'. Move probes into the skill body via runtime Bash tool calls instead. Found: ${JSON.stringify(preResolutions)}`,
    ).toBeNull()
  })
  ...
  test("instructs the agent to invoke each probe script with a CLAUDE_SKILL_DIR-prefixed path", () => {
    for (const script of ["upstream-version.sh", "currently-loaded-version.sh", "marketplace-name.sh"]) {
      expect(
        ...
        `sl-update/SKILL.md must instruct the agent to run 'bash "\${CLAUDE_SKILL_DIR}/scripts/${script}"' — relative paths like 'bash scripts/${script}' fail at runtime because the Bash tool's CWD is the user's project, not the skill directory.`,
      ).toBe(true)
    }
  })
  ...
  test("declares narrow allowed-tools patterns for each probe script", () => {
    ...
    expect(frontmatter, "sl-update/SKILL.md must have YAML frontmatter").not.toBeNull()
    const allowedTools = frontmatter![1].match(/^allowed-tools:\s*(.+)$/m)
    expect(
      allowedTools,
      "sl-update/SKILL.md must declare `allowed-tools:` for each probe script so users without bypassPermissions don't get a prompt every run.",
    ).not.toBeNull()
    const tools = allowedTools![1]
    for (const script of ["upstream-version.sh", "currently-loaded-version.sh", "marketplace-name.sh"]) {
      expect(
        tools.includes(`Bash(bash *${script})`),
        `sl-update/SKILL.md allowed-tools must include 'Bash(bash *${script})' so the runtime Bash call passes the permission check without granting blanket Bash access (got: ${tools})`,
      ).toBe(true)
    }
    expect(
      /Bash\(bash \*\)/.test(tools),
      `sl-update/SKILL.md allowed-tools must NOT use the broad 'Bash(bash *)' pattern — pin to each script filename instead (got: ${tools})`,
    ).toBe(false)
  })
})
...
// own location.
describe("sl-update probe scripts are self-locating", () => {
  function runFromFakeMarketplace(scriptName: string, marketplaceName: string, version: string): string {
    const root = mkdtempSync(path.join(tmpdir(), "sl-update-fake-marketplace-"))
    try {
      const skillDir = path.join(root, ".claude/plugins/cache", marketplaceName, "super-looper", version, "skills/sl-update")
...
// Regression guard for https://github.com/akornmeier/super-looper/issues/659.
//
...
describe("sl-update upstream-version.sh script", () => {
...
function runUpstreamScript(scriptPath: string, options: MockOptions): string {
  ...
  const mockDir = mkdtempSync(path.join(tmpdir(), "sl-update-gh-"))
  try {
    const pluginJsonB64 = pluginJsonVersion
      ? Buffer.from(
          JSON.stringify({ name: "super-looper", version: pluginJsonVersion }),
        ).toString("base64")
      : ""
    const releaseJson = releaseTagVersion
      ? JSON.stringify([{ tagName: `super-looper-v${releaseTagVersion}` }])
      : "[]"
