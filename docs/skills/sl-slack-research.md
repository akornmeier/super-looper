...
`sl-slack-research` is the **organizational-context retrieval** skill. It dispatches `sl-slack-researcher` to search Slack for context relevant to a topic, then synthesizes a research digest with cross-cutting analysis and a research-value assessment — not a flat list of message hits. Useful when planning, brainstorming, or any task where the team has already discussed something and the agent needs that context to make sensible recommendations.
...
| Key contrast | `sl-slack-research` synthesizes; `slack:find-discussions` returns raw message hits |
...
`sl-slack-research` runs Slack search through a research-shaped pipeline:
...
- **Single-shot dispatch** — the skill is a thin entry point; `sl-slack-researcher` does the actual work (MCP discovery, search execution, thread reads, synthesis)
...
The skill itself is a thin entry point that asks for a topic if one wasn't given, then dispatches `sl-slack-researcher`. The agent handles MCP discovery, search execution, thread reads, and synthesis. This keeps the user-facing surface tiny while the heavy lifting stays in a specialist agent.

### 6. Distinct from raw-search Slack tools

`slack:find-discussions` (different skill) returns individual message results suitable for "did anyone mention X recently?" `sl-slack-research` returns interpreted context suitable for "what does the team think about X, with what evidence?" The distinction matters — using the wrong tool gets noise back.
...
You're about to plan a free-trial feature. You want to know what the team has already discussed before writing the brainstorm. You invoke `/sl-slack-research "free trial in #proj-reverse-trial"`.

The skill dispatches `sl-slack-researcher` with the topic, channel scope, and an implicit recent-window. The agent connects to the configured Slack workspace, runs targeted searches in `#proj-reverse-trial`, expands relevant threads, and synthesizes findings.
...
You now have real organizational context before invoking `/sl-brainstorm`. The brainstorm starts from "the trial is 14-day, soft-paywall, no pricing visible — when does the gate fire?" rather than from scratch.

---

## When to Reach For It

Reach for `sl-slack-research` when:
...
`sl-slack-research` plugs in upstream of planning and brainstorming when organizational context matters:

- **Before `/sl-brainstorm`** — gather Slack context so the brainstorm's pressure-test questions can ground in real prior discussion
- **Before `/sl-plan`** — when the work touches a decision the team made elsewhere, surface that decision first
- **From inside `/sl-plan` Phase 1.1** — opt-in via "Slack tools detected; ask me to search Slack for organizational context"
- **From inside `/sl-brainstorm` Phase 1.1** — opt-in via the same surfacing pattern
- **From inside `/sl-ideate` Phase 1** — opt-in slack-research for the ideation grounding step
...
- **Topic** — `/sl-slack-research free trial`
- **Question** — `/sl-slack-research "What did we say about free trial recently?"`
- **Channel-scoped** — `/sl-slack-research free trial in:#proj-reverse-trial`
- **Date-filtered** — `/sl-slack-research onboarding flow after:2026-03-01`
- **No argument** — `/sl-slack-research` asks what topic to research
...
`slack:find-discussions` returns individual message results — useful for "did anyone mention X recently?" `sl-slack-research` interprets and synthesizes — useful for "what does the team think about X, with what evidence?" Use the right tool for the question.
...
- [`sl-brainstorm`](./sl-brainstorm.md) — opt-in slack-research during Phase 1.1 constraint check
- [`sl-plan`](./sl-plan.md) — opt-in slack-research during Phase 1.1 local research
- [`sl-ideate`](./sl-ideate.md) — opt-in slack-research during grounding
