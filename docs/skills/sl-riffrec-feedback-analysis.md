# `sl-riffrec-feedback-analysis`

> Turn raw [Riffrec](https://github.com/kieranklaassen/riffrec) recordings into structured product feedback — quick bug reports for short captures, extensive analysis for longer ones, with handoff to `sl-brainstorm` when requirements emerge.

`sl-riffrec-feedback-analysis` is the **product-feedback consumption** skill. Riffrec is a separate capture tool that records synchronized screen + voice + event sessions and emits a `riffrec-*.zip` bundle. This skill is the consumption side: it analyzes those bundles (or any video / audio / meeting-notes file), routes between three paths based on length and intent, and produces the right artifact for what's actually inside the recording.
...
| What it produces | Quick: one concise bug report. Extensive: requirements-shaped analysis + handoff to `/sl-brainstorm` |
...
`sl-riffrec-feedback-analysis` runs a routed flow with three paths:
...
- **Extensive analysis** — longer recording or multiple issues → structured analysis with screenshots, requirements-shaped output, mandatory handoff to `/sl-brainstorm`
...
- **Extensive analysis** — longer, multiple issues / requirements / workflow walkthroughs, or the user wants requirements material. Output is the full structured analysis with screenshots; the skill **always continues into `/sl-brainstorm`**.
...
In repos with `docs/brainstorms/`, the default output directory is `docs/brainstorms/riffrec-feedback/` so the analysis lands where downstream skills (`sl-brainstorm`, `sl-plan`) expect to find requirements material. The quick path overrides to a temp location so it doesn't pollute the repo with single-issue bug reports.

### 5. Super Looper output format for extensive analysis

The extensive path produces output in the Super Looper feedback format documented in `references/super-looper-feedback-format.md` — structured to feed cleanly into `/sl-brainstorm` as raw input. Multiple issues / requirements / observations get separated, each with the relevant screenshot frames and timestamps. The brainstorm receives a starting point, not a transcript.

### 6. Mandatory `sl-brainstorm` handoff for extensive analysis

The extensive path always continues into `/sl-brainstorm` after the analysis lands. The recording captured *what the user experienced*; the brainstorm clarifies *what to build in response*. Without the handoff, the analysis sits on disk and nobody knows what to do with it.
...
The skill loads `/sl-brainstorm` with the analysis as the starting point. Brainstorm clarifies which issue to address first, what success looks like, and produces the requirements doc.

---

## When to Reach For It

Reach for `sl-riffrec-feedback-analysis` when:
...
- A long walkthrough recording needs to become structured input for `/sl-brainstorm`

Skip `sl-riffrec-feedback-analysis` when:

- The feedback is text-only and short — just paste it directly into `/sl-brainstorm`
...
The extensive path always continues into `/sl-brainstorm` so the captured feedback becomes a real artifact downstream skills can use. The quick path produces a complete artifact on its own (a bug report) without forcing brainstorm overhead.
...
- **Riffrec zip** — `/sl-riffrec-feedback-analysis riffrec-2026-05-04-checkout-flow.zip`
- **Video file** — `/sl-riffrec-feedback-analysis demo.mp4`
- **Audio file** — `/sl-riffrec-feedback-analysis voice-memo.m4a`
- **Meeting notes** — `/sl-riffrec-feedback-analysis meeting-notes.md`
- **Setup question** — `/sl-riffrec-feedback-analysis "how do I install riffrec"` (no input file; routes to setup path)
...
Analyzer: `scripts/analyze_riffrec_zip.py`. Super Looper output format: `references/super-looper-feedback-format.md`.
...
**Why does the extensive path always continue into `/sl-brainstorm`?**
...
- [`sl-brainstorm`](./sl-brainstorm.md) — extensive-analysis output feeds directly into brainstorm
- [`sl-plan`](./sl-plan.md) — downstream of brainstorm; receives the requirements doc that came from the recording
- [`sl-debug`](./sl-debug.md) — when a quick-path bug report has a clear root cause to investigate
