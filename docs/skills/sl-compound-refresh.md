...
| When to use it | After significant refactors; when `sl-compound` flags an older doc as superseded; when learnings are accumulating drift; periodic hygiene sweeps |
...
`sl-compound-refresh` runs as a structured review with five explicit outcomes:
...
Like `sl-compound`, every refresh run checks whether `AGENTS.md`/`CLAUDE.md` surfaces `docs/solutions/`. The check runs every time — knowledge only compounds value when agents can find it. In autofix mode, the recommendation appears in the report rather than being applied (autofix scope is doc maintenance, not project config).

---

## Quick Example

You've just merged a refactor that renamed several models in the auth subsystem. You invoke `/sl-compound-refresh auth`.
...
Reach for `sl-compound-refresh` when:

- A significant refactor or rename just landed and learnings in that area likely drifted
- `sl-compound` flagged a specific older doc as superseded by a new learning
...
- You're in the middle of a debugging or build session — capture first via `/sl-compound`, refresh later

---

## Use as Part of the Workflow

`sl-compound-refresh` is the maintenance counterpart to `/sl-compound`:

- **Triggered from `/sl-compound`** — Phase 2.5's selective refresh check passes a narrow scope hint when a new learning suggests an older doc may be stale
- **Manual periodic invocation** — typically scoped (`/sl-compound-refresh auth`, `/sl-compound-refresh performance-issues`) to avoid sweeping reviews without evidence
...
The pairing matters: `sl-compound` adds new docs; `sl-compound-refresh` ensures the existing set stays lean. Without the second, the first eventually clutters.
...
- **Specific file** — `/sl-compound-refresh plugin-versioning-requirements`
- **Module/component** — `/sl-compound-refresh payments`
- **Category** — `/sl-compound-refresh performance-issues`
- **Pattern topic** — `/sl-compound-refresh critical-patterns`
- **Autofix mode** — `/sl-compound-refresh auth mode:autofix` (no user interaction; report is the deliverable)
- **Broad sweep** (rare) — `/sl-compound-refresh` with no scope, processes everything
...
- [`sl-compound`](./sl-compound.md) — captures new learnings; this skill maintains the existing set
- [`sl-plan`](./sl-plan.md) — reads `docs/solutions/` as institutional memory; benefits from clean, current docs
- [`sl-ideate`](./sl-ideate.md) — also consults `docs/solutions/` during grounding
- [`sl-doc-review`](./sl-doc-review.md) — different skill: persona-based review of a single doc, not maintenance across the set
