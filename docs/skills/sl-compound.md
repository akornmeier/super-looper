...
| What's next | Optional `/sl-compound-refresh` if the new learning suggests an older doc may be stale |
...
`sl-compound` runs as a structured capture flow at the moment context is freshest:
...
**Full mode** runs three research subagents in parallel (Context Analyzer / Solution Extractor / Related Docs Finder), then optionally a foreground Session Historian (off by default — searches your prior Claude Code sessions for related context). Cross-references existing docs, detects duplicates, runs specialized reviews.
...
- **Moderate overlap** (2-3 dimensions match) → create the new doc, flag for consolidation review (potential `sl-compound-refresh` trigger).
...
After capturing the new learning, `sl-compound` checks whether it should invoke `/sl-compound-refresh` on a narrow scope hint. It does NOT default to running refresh — only when the new learning suggests a specific older doc may now be stale (contradicted, superseded, or in a domain that just got refactored).

### 6. Specialized post-review

Based on the problem type, optional specialized agents review the documentation: `sl-performance-oracle` for performance issues, `sl-security-sentinel` for security, `sl-data-integrity-guardian` for database, and `sl-code-simplicity-reviewer` for code-heavy issues.

### 7. Session history integration (opt-in)

Full mode optionally dispatches `sl-session-historian` to search prior sessions across harnesses for related context — what was tried before, what didn't work, key decisions. Findings are folded into "What Didn't Work" (bug track) or "Context" (knowledge track). Off by default because of token cost; the user explicitly opts in.

### 8. Auto-invoke triggers

Phrases like "that worked", "it's fixed", "working now", "problem solved" auto-invoke the skill so capture happens at the moment context is freshest. The user can override with `/sl-compound [context]` to capture immediately.
...
`sl-compound` auto-invokes (or you call it explicitly). It asks whether to use Full or Lightweight mode, then whether to also search session history. You pick Full, no session history.
...
Phase 3 dispatches `sl-performance-oracle` and `sl-code-simplicity-reviewer` to validate the code examples and approach. Phase 2.5 surfaces a refresh recommendation: the older N+1 doc may benefit from consolidation review. The skill suggests `/sl-compound-refresh n-plus-one` as a narrow scope hint and ends.

---

## When to Reach For It

Reach for `sl-compound` when:
...
`sl-compound` is the closing loop of multiple workflows:

- **`/sl-debug` Phase 4** — after a successful fix and PR, optionally offers `sl-compound` when the bug is generalizable (3+ recurrence, wrong assumption about a shared dependency)
- **`/sl-work` Phase 4** — after shipping, surfaces `sl-compound` when the work yielded a reusable pattern, convention, or tooling decision
- **Stand-alone** — invoked directly after any non-trivial problem-solving session

The output feeds back into upstream skills:

- `/sl-plan` reads `docs/solutions/` via `sl-learnings-researcher` during Phase 1 research
- `/sl-ideate` reads it as part of the comprehensive grounding step
- `/sl-debug` reads it for prior context when an issue tracker reference is fetched

When the new learning suggests an older doc may now be stale, `sl-compound` recommends `/sl-compound-refresh` with a narrow scope hint.
...
- **Just-finished problem** — `/sl-compound` (or auto-invoked from "that worked")
- **With context hint** — `/sl-compound "the email digest race condition we fixed"`
...
- [`sl-compound-refresh`](./sl-compound-refresh.md) — maintain `docs/solutions/` over time as the codebase evolves
- [`sl-debug`](./sl-debug.md) — common upstream caller after a fix is verified
- [`sl-work`](./sl-work.md) — common upstream caller after shipping
- [`sl-plan`](./sl-plan.md) — reads `docs/solutions/` as institutional memory during planning
- [`sl-ideate`](./sl-ideate.md) — reads `docs/solutions/` as part of grounding
