...
If no blocking question tool is available (background agent, or any mode where there is no synchronous user), skip Steps 1–2 and go straight to headless upload:
...
Present the preview URL to the user and ask how to handle the evidence. Use `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to presenting options in chat only if it is unavailable or the call errors. Never silently skip the question.
...
3. **Save locally** -- save to a stable OS-temp path (/tmp/super-looper/sl-demo-reel/)
...
```bash
KEY="sl-demo-reel/$(date +%Y%m%d-%H%M%S)-$(basename [ARTIFACT_PATH])"
...
Return the structured output defined in the SKILL.md Output section: `Tier`, `Description`, and either `URL` (permanent public URL) or `Path` (local file path). The caller formats the evidence into the PR description. sl-demo-reel does not generate markdown.
