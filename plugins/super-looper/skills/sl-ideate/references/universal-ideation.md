...
Then offer the Phase 5 next-steps menu via `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to a numbered list only if it is unavailable or the call errors. Never silently skip. Four options, option 1 format-keyed:
...
2. **Brainstorm one idea with `sl-brainstorm`** — go deeper on one chosen idea (asks which). In universal mode this is **not** the first step of an implementation chain — there is no `sl-plan` → `sl-work` after; `sl-brainstorm` develops the idea further (a name into a brand brief, a plot into an outline, a decision into a weighed framework) and ends there. Seed it with the idea's substance + a provenance pointer (per §5.2) — not the whole file.
