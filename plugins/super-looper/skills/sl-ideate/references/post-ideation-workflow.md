...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `docs/ideation/`):** write into the run's CE temp area — the `<scratch-dir>` resolved in Phase 1 (`/tmp/super-looper/sl-ideate/<run-id>/`). Do **not** write into the user's current working directory, and do **not** create a `docs/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note it is temporary (`/tmp` is cleared on reboot — move it to keep it).
...
Ask what to do next using `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to numbered options in chat only if it is unavailable or the call errors. Never silently skip the question. Free-text answers are accepted.
...
2. **Brainstorm one idea with `sl-brainstorm`** — commit a chosen idea to a requirements doc; leaves sl-ideate. Asks which idea first.
...
- **Markdown — Open and iterate in Proof.** The local markdown file already exists (Phase 4), so Proof is a review surface over it, not the primary record. Load the `sl-proof` skill in HITL-review mode with:
  - **source file:** the saved `.md` file from Phase 4.
  - **doc title:** `Ideation: <topic>` or the doc's H1.
  - **identity:** `ai:super-looper` / `Super Looper`.
  - **recommended next step:** `/sl-brainstorm`.
...
2. **Build a focused seed** from the idea's substance already in the orchestrator's context. Do **not** pass the whole file — wasteful and noisy (the other survivors, grounding, and rejection table are irrelevant to defining this one idea, and an HTML file carries CSS/SVG chrome). Do **not** pass only a file pointer — that forces `sl-brainstorm` to re-open and re-extract the idea the orchestrator already holds. The seed is feature-description-shaped:

   > `<title> — <description>. Basis: <basis/evidence>. Why it matters: <rationale>. Known tradeoffs: <downsides>.`

   The basis/evidence directly feeds `sl-brainstorm`'s product-pressure-test, so it won't re-derive what we already know. Append a one-line provenance pointer: `(Seeded from sl-ideate: <path>, idea "<title>")` — it records origin and lets brainstorm pull adjacent detail if it wants, without being forced to read anything.
3. **Load the `sl-brainstorm` skill** with that seed. The saved file is already the record — no extra write step.

**Repo mode only:** do **not** skip brainstorming and go straight to `sl-plan` — `sl-plan` wants brainstorm-grounded requirements. In elsewhere modes, ideation is a legitimate terminal state; brainstorming is optional deeper development of one idea, not a required next rung on an implementation ladder that does not exist in these modes.

### 5.3 Iterate on One Idea

This stays in sl-ideate — no skill handoff. It is the "poke at one idea before committing" step.
...
- acting on an idea routes to `sl-brainstorm` (with a substance seed, not the whole file), not directly to implementation
