...
describe("sl-code-review contract", () => {
  test("documents explicit modes and orchestration boundaries", async () => {
    const content = await readRepoFile("plugins/super-looper/skills/sl-code-review/SKILL.md")
    ...
    expect(content).toContain("/tmp/super-looper/sl-code-review/<run-id>/")
  ...
  test("documents policy-driven routing and actionable handoff", async () => {
    ...
    expect(content).not.toContain("sl-todo-create")
    ...
    const subagentTemplate = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/subagent-template.md",
    )
  ...
  test("keeps findings schema and downstream docs aligned", async () => {
    const rawSchema = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/findings-schema.json",
    )
  ...
  test("action-class rubric defines caller routing without safe_auto", async () => {
    const rubric = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/action-class-rubric.md",
    )
  ...
  test("Stage 4 spawning restates model-override imperative at point of action", async () => {
    ...
    expect(content).toMatch(/sl-correctness-reviewer.*sl-security-reviewer.*sl-adversarial-reviewer/s)
    ...
    // with the dispatch primitive named at the call site.
    ...
    expect(spawning).toContain("sl-correctness-reviewer")
    expect(spawning).toContain("sl-security-reviewer")
    expect(spawning).toContain("sl-adversarial-reviewer")
  })
  ...
  test("Stage 5b validation pass dispatches conditionally and bounds parallelism", async () => {
    ...
    const validatorTemplate = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/validator-template.md",
    )
  ...
  test("Stage 5c applies safe fixes in default mode, report-only in mode:agent, no deny-list", async () => {
    const content = await readRepoFile("plugins/super-looper/skills/sl-code-review/SKILL.md")
    const template = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/review-output-template.md",
    )
  ...
  test("remote scope modes forbid workspace inspection on wrong tree", async () => {
    const skill = await readRepoFile("plugins/super-looper/skills/sl-code-review/SKILL.md")
    const diffScope = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/diff-scope.md",
    )
  ...
  test("personas use anchored rubric language and no float references remain", async () => {
    const personas = [
      ...
      "sl-agent-native-reviewer",
    ]

    for (const persona of personas) {
      const content = await readRepoFile(`plugins/super-looper/agents/${persona}.md`)
  ...
  test("documents stack-specific conditional reviewers for the JSON pipeline", async () => {
    ...
    const catalog = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/persona-catalog.md",
    )

    for (const agent of ["sl-julik-frontend-races-reviewer", "sl-swift-ios-reviewer"]) {
      expect(content).toContain(agent)
      expect(catalog).toContain(agent)
    }

    for (const removed of [
      "sl-dhh-rails-reviewer",
      "sl-kieran-rails-reviewer",
      "sl-kieran-python-reviewer",
      "sl-kieran-typescript-reviewer",
    ]) {
  ...
  test("stack-specific reviewer agents follow the structured findings contract", async () => {
    const reviewers = [
      {
        path: "plugins/super-looper/agents/sl-julik-frontend-races-reviewer.md",
        reviewer: "julik-frontend-races",
      },
      {
        path: "plugins/super-looper/agents/sl-swift-ios-reviewer.md",
        reviewer: "swift-ios",
      },
    ]
  ...
  test("JSON-pipeline persona agents grant Write so they can save run artifacts", async () => {
    // The sl-code-review subagent template instructs each persona to write its full
    // analysis to /tmp/super-looper/sl-code-review/{run_id}/{reviewer}.json.
  ...
  test("data-migration reviewer consolidates schema drift and migration safety", async () => {
    const content = await readRepoFile(
      "plugins/super-looper/agents/sl-data-migration-reviewer.md",
    )
    const skill = await readRepoFile("plugins/super-looper/skills/sl-code-review/SKILL.md")
    ...
    expect(skill).not.toContain("sl-schema-drift-detector")
    expect(skill).not.toContain("sl-data-migration-expert")
    expect(skill).not.toContain("sl-data-migrations-reviewer")
  })
  ...
  test("sl-work documents review-findings followup after Tier 2", async () => {
    ...
    // cold-caller fallback only (it must not start a second review in the sl-work Tier 2 path).
  ...
  test("sl-work shipping-workflow enforces a residual-work gate after Tier 2 review", async () => {
    for (const path of [
      ...
      expect(workflow).toContain("If the user later chooses the no-PR `sl-commit` path")
  ...
  test("lfg autonomously handles residuals via non-interactive tracker-defer and PR description", async () => {
    ...
    await expect(readRepoFile("plugins/super-looper/skills/lfg/references/tracker-defer.md")).resolves.not.toMatch(
      /no-sink/,
    )
    ...
    const followup = await readRepoFile("plugins/super-looper/skills/lfg/references/review-followup.md")
    ...
    expect(lfg).not.toContain("plugins/super-looper/skills/sl-code-review/references/tracker-defer.md")
    ...
    // falsely trip on step 7's legitimate use of sl-commit-push-pr for the
  ...
  test("sl-code-review emits actionable findings summary for callers", async () => {
    const content = await readRepoFile("plugins/super-looper/skills/sl-code-review/SKILL.md")
  ...
  test("sl-code-review uses stable sequential finding numbers across grouped output", async () => {
    const content = await readRepoFile("plugins/super-looper/skills/sl-code-review/SKILL.md")
    const template = await readRepoFile(
      "plugins/super-looper/skills/sl-code-review/references/review-output-template.md",
    )
    const fixture = await readRepoFile("tests/fixtures/sl-code-review-stable-numbering.md")
...
describe("testing-reviewer contract", () => {
  test("includes behavioral-changes-with-no-test-additions check", async () => {
    const content = await readRepoFile("plugins/super-looper/agents/sl-testing-reviewer.md")
