...
| What's next | `/sl-ideate`, `/sl-brainstorm`, `/sl-plan`, or `/sl-product-pulse` — all consult the doc as grounding |
...
`sl-strategy` runs an interview with explicit pushback rules:

- **Anchor, not plan** — strategy is what the product is and why; features belong in `sl-brainstorm`, schedules belong in the issue tracker
...
- **`sl-ideate`** — codebase-scan grounding agent reads it; ideation weights toward strategy-aligned directions automatically
- **`sl-brainstorm`** — Phase 1.1 constraint check reads it; product/scope decisions stay anchored to active tracks
- **`sl-plan`** — repo-research-analyst reads it; plan flags decisions that pull away from active tracks or the stated approach
- **`sl-product-pulse`** — first-run interview seeds product name and key metrics from the doc, then wires up data sources to actually measure those metrics
...
You're starting a new product and want a strategy doc before kicking off `sl-ideate`. You invoke `/sl-strategy`.
...
Phase 3 notes the doc is now in place and `sl-ideate`, `sl-brainstorm`, `sl-plan`, and `sl-product-pulse` will pick it up on their next run. Suggests `/sl-ideate` as a natural next step.

---

## When to Reach For It

Reach for `sl-strategy` when:
...
- A specific section feels weak and you want to revisit it (`/sl-strategy approach`)
- A downstream skill (`sl-ideate`, `sl-brainstorm`) flagged the absence of `STRATEGY.md` as missing grounding

Skip `sl-strategy` when:

- The strategy is on file and still accurate — re-running adds noise without value
- You're trying to plan a single feature → `/sl-brainstorm`
...
The downstream skills don't *require* `STRATEGY.md` — they work without it. But when the doc exists, the active tracks and stated approach pull ideation, brainstorming, and planning toward strategy-aligned directions automatically. When `STRATEGY.md` is absent, `sl-ideate` can still ground in the codebase, but it has no signal about what *kind* of work matters most right now.

`sl-product-pulse` similarly seeds its first-run interview from `STRATEGY.md`'s key metrics — wiring up data sources to measure what the strategy says matters.
...
- **First run** — `/sl-strategy` (no `STRATEGY.md` exists)
- **Targeted update** — `/sl-strategy approach` jumps directly to that section
- **Open update** — `/sl-strategy` (file exists, no argument) asks which section(s) to revisit
...
Because long strategy docs aren't read. The discipline forces sharp answers to a small number of questions. If you find yourself wanting more sections, the answer is usually "those belong in sl-brainstorm or the issue tracker, not in strategy."
...
- [`sl-ideate`](./sl-ideate.md) — reads `STRATEGY.md` as grounding for ideation
- [`sl-brainstorm`](./sl-brainstorm.md) — reads it for constraint awareness during scope work
- [`sl-plan`](./sl-plan.md) — reads it; flags plan decisions that pull away from active tracks
- [`sl-product-pulse`](./sl-product-pulse.md) — seeds first-run setup from the strategy's key metrics
