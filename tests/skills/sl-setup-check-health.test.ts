...
describe("sl-setup check-health", () => {
  test("detects global skills under ~/.claude/skills when skills CLI misses them", async () => {
    const root = await mkdtemp(path.join(os.tmpdir(), "sl-setup-health-"))

    try {
      await mkdir(path.join(root, ".claude", "skills", "ast-grep"), { recursive: true })
