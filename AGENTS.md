...
- **Merge policy:** Pull requests are the required path for every change to `main`; treat a direct push as forbidden regardless of what the ruleset currently permits — a bypass actor still exempts the maintainer today. The intended enforcement contract — a pull-request rule plus the `test`, `pr-title`, `pr-size`, and `pr-reviewed` status checks — is specified in `docs/solutions/workflow/main-ruleset-contract.md`. Do not trust this file's prose for what is live — verify the actual state with `gh api repos/akornmeier/super-looper/rulesets/17763543`. The direct path skips `release:validate`, the test suite, and PR title validation; past direct merges caused version drift requiring multi-PR recovery (see `docs/solutions/workflow/release-please-version-drift-recovery.md`). Read the contract doc before renaming a workflow job, since a required context is matched by literal name and a rename blocks every pull request.
