...
`sl-clean-gone-branches` runs cleanup in three stages:
...
You haven't cleaned local branches in a while. You invoke `/sl-clean-gone-branches`.
...
Reach for `sl-clean-gone-branches` when:
...
Skip `sl-clean-gone-branches` when:
...
`sl-clean-gone-branches` is mostly standalone — it doesn't sit inside the chain. It's invoked when:
...
- [`/sl-worktree`](./sl-worktree.md) — sibling skill for worktree creation; this skill cleans up after worktrees become orphaned
