...
This is the first step in the super-looper ideation chain:
...
The chain works across domains — every step supports universal mode. `sl-ideate` is the upstream "find the strong candidates" step, but it's a complete cycle on its own.
...
| What it produces | Ranked ideation artifact written as a single self-contained HTML file by default (humans are the audience — rich, openable in a browser); pass `output:md` for markdown. Written automatically to `docs/ideation/` when present, else an announced temp path under `/tmp/super-looper/` |
| What's next | `/sl-brainstorm` on a chosen survivor, iterate on one first, or just keep the saved file |
...
`sl-ideate` separates **grounding**, **generation**, **critique**, and **selection** as discrete phases — and the quality mechanism is **explicit rejection with reasons**, not optimistic ranking.
...
`/sl-ideate "surprise me"` skips the subject step entirely. Sub-agents discover their own subjects from grounding material. Different frames finding different subjects is the feature, not a bug — cross-cutting combinations across discovered subjects often produce the strongest ideas.
...
You invoke `sl-ideate "DX improvements"` from inside a code repo. The agent announces it'll dispatch ~13 agents — most on cheap tiers — and offers skip phrases for cost control.
...
- You already have a specific feature or decision in mind → `/sl-brainstorm`
- Requirements are ready and you need execution guardrails → `/sl-plan`
- You're debugging a known bug → `/sl-debug`
...
```text
/sl-ideate            "What's worth exploring?"
   |
...
Each artifact is structured input for the next: the survivor's basis carries forward as the brainstorm's evidence base; the brainstorm's decisions flow into the plan's requirements and scope; the plan's U-IDs and test scenarios become the guardrails `sl-work` executes against. When you pick "Brainstorm one idea" in the next-steps menu, `sl-brainstorm` loads with that idea as a substance seed (its basis, rationale, and tradeoffs) — the ideation file is already saved.
...
`sl-ideate` is a complete ideation cycle on its own — it produces a ranked, reasoned idea set as a saved file you can open, share, brainstorm from, or discard.

**Software:**

- **Codebase audits** — `/sl-ideate "what to improve in this repo"` (pair with `STRATEGY.md` for strategy-aligned weighting)
- **Issue triage** — `/sl-ideate "biggest issue themes in the last quarter"`
- **Pricing or positioning ideation** — `/sl-ideate "pricing page A/B test ideas"`
- **Surprise-me runs on any subject** — `/sl-ideate "surprise me"` from inside any repo
...
| `output:md` | Write the artifact as markdown instead of the default self-contained HTML (`output:html` forces HTML explicitly). Also settable per-project via `ideate_output` in `.super-looper/config.local.yaml` |
...
- [`sl-brainstorm`](./sl-brainstorm.md) — once you've picked a survivor, brainstorm the chosen direction into a requirements doc
- [`sl-plan`](./sl-plan.md) — once requirements are clear, plan the implementation
- [`sl-strategy`](./sl-strategy.md) — anchor ideation to a documented product strategy
- [`sl-doc-review`](./sl-doc-review.md) — review the saved ideation artifact for clarity and completeness (markdown output only — run with `output:md` first)
- [`sl-proof`](./sl-proof.md) — open the artifact in Proof for collaborative iteration (markdown output only — Proof can't ingest HTML)
