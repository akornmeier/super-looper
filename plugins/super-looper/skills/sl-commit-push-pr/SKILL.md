...
**Asking the user:** When this skill says "ask the user", use `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to presenting the question in chat only if it is unavailable or the call errors. Never silently skip the question.
...
- **Capture now** — load `sl-demo-reel` with a target description from the branch diff. It returns `Tier`, `Description`, `URL`, `Path`. Exactly one of `URL`/`Path` contains a real value; the other is `"none"`. If `URL`, splice as a `## Demo` section. If `Path` (user chose local save), note in the body that a demo was recorded but is not embedded. If skipped, proceed without evidence.
...
```bash
BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/sl-pr-body.XXXXXX") && cat > "$BODY_FILE" <<'__CE_PR_BODY_END__'
