...
| What it produces | Committed fixes on the current branch (no PR by default — use `/sl-commit-push-pr` after) |
...
The skill detects the host IDE (Claude Code, VS Code) via env-var probes (`references/ide-detection.md`) and opens the dev server URL using the matching mechanism: `open` for Claude Code, VS Code's Simple Browser. The right surface for the right environment, no manual juggling.
...
`disable-model-invocation: true` in the frontmatter prevents the skill from auto-triggering. Polish is a deliberate user choice — the skill only fires when you type `/sl-polish` directly. This avoids surprising the user when they just wanted to look at a page.

---

## Quick Example

You've just finished a notification settings page. It works, but the spacing feels off, the toggle states aren't quite right, and the empty-state copy is dry. You invoke `/sl-polish`.

The skill verifies you're on a feature branch (not main), checks for `.claude/launch.json` (none), runs `detect-project-type.sh` (detects `next`), reads `references/dev-server-next.md` for the start command, resolves your package manager (pnpm) via `resolve-package-manager.sh`, picks port 3000, and starts `pnpm dev` in the background. After 4 seconds, `localhost:3000` responds. The skill opens it in VS Code's Simple Browser.
...
You're happy. The agent commits the fixes. You move on with `/sl-commit-push-pr`.

---

## When to Reach For It

Reach for `sl-polish` when:
...
- The feature isn't built yet → use `/sl-work`
- The polish needs design specs (Figma comparison, brand-system alignment) → use `/sl-frontend-design` or a dedicated design-sync skill
...
`sl-polish` is invoked late, after a feature is functionally complete:

```text
/sl-work or /sl-debug → feature works → /sl-polish → /sl-commit-push-pr
```

It doesn't have direct callers in the chain — polish is a deliberate user invocation when the work needs visual refinement. After the polish loop ends, the standard shipping handoff is `/sl-commit-push-pr` to open the PR.
...
- **Current branch** — `/sl-polish`
- **Specific PR** — `/sl-polish 1234` (checks out the PR)
- **Specific branch** — `/sl-polish feat/notification-settings`
...
The skill detects VS Code via env-var probes and uses its preferred browser surface. Outside that, it falls back to `open`. The framework detection and dev-server start are IDE-agnostic.

**Why no PR creation at the end?**
Polish often needs more than one session, and forcing a PR every time would clutter. Commit-and-PR is a separate user choice via `/sl-commit-push-pr`.

---

## See Also

- [`sl-work`](./sl-work.md) — build the feature first; polish second
- [`sl-commit-push-pr`](./sl-commit-push-pr.md) — open the PR after polish is done
- [`sl-frontend-design`](./sl-frontend-design.md) — for high-quality frontend design from scratch (different scope)
- [`sl-debug`](./sl-debug.md) — for fixing bugs you find during polish, when root-cause investigation matters
