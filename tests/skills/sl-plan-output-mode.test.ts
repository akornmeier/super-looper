...
const HTML_RENDERING_PATH = path.join(
  process.cwd(),
  "plugins/super-looper/skills/sl-plan/references/html-rendering.md",
)
...
// Regression guard for the `output:html` / `output:md` argument on sl-plan.
...
describe("sl-plan output:html mode", () => {
  test("argument-hint advertises output:html", () => {
    ...
    expect(
      ...
      `sl-plan argument-hint must mention 'output:html' so humans discover the flag. Current value: ${JSON.stringify(hint)}`,
    ).toBe(true)
  })
  ...
  test("SKILL.md describes the Output Mode resolution inline (not solely in a reference)", () => {
    ...
    expect(
      phaseStart,
      "sl-plan SKILL.md no longer contains the Phase 0.0 anchor — Output Mode resolution was removed or moved without updating the test.",
    ).toBeGreaterThan(-1)
  ...
  test("config matching rule ignores commented YAML lines (active-key principle)", () => {
    // A review (2026-05-13, thread PRRT_kwDOP_gZVc6B6OgB) flagged that the
  ...
  test("unknown-value fallback note reflects final resolved mode, not a hardcoded md", () => {
    // A review (2026-05-13, thread PRRT_kwDOP_gZVc6B-LIW) flagged that
  ...
  test("Phase 5.2 names the HTML/sl-doc-review timing relationship", () => {
    // Composition timing rule: Phase 5.2 must surface the relationship between
    // HTML emission and sl-doc-review so an agent doesn't ship a pre-review
    // artifact without understanding the gap. sl-doc-review's mutation mechanics
    ...
    // by naming the format gate, calling out that sl-doc-review is skipped on
    ...
    expect(
      /skipped in HTML|markdown-only|format gate|Phase 5\.3\.8|safe_auto|after.*sl-doc-review/i.test(
        phase52Region,
      ),
      "Phase 5.2 must surface the HTML/sl-doc-review relationship inline — either naming the 5.3.8 format gate that skips sl-doc-review on HTML, or stating that sl-doc-review is markdown-only today, so an agent composing the HTML knows what the artifact does and does not reflect.",
    ).toBe(true)
  })
  ...
  test("plan-sections.md enumerates the required plan metadata fields by name", () => {
    ...
    // removed (sl-work no longer mutates the plan; completion is derived from
