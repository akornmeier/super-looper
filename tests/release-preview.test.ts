...
describe("release preview", () => {
  test("uses changed files to determine affected components and next versions", async () => {
    ...
    const preview = await buildReleasePreview({
      title: "fix: adjust sl-plan wording",
      files: ["plugins/super-looper/skills/sl-plan/SKILL.md"],
    })

    expect(preview.components).toHaveLength(1)
    expect(preview.components[0].component).toBe("super-looper")
    expect(preview.components[0].inferredBump).toBe("patch")
    expect(preview.components[0].nextVersion).toBe(bumpVersion(versions["super-looper"], "patch"))
  })

  test("supports per-component overrides without affecting unrelated components", async () => {
    const versions = await loadCurrentVersions()
    const preview = await buildReleasePreview({
      title: "fix: refine super-looper prompts",
      files: ["plugins/super-looper/README.md"],
      overrides: {
        "super-looper": "minor",
      },
    })
    ...
    expect(preview.components[0].nextVersion).toBe(bumpVersion(versions["super-looper"], "minor"))
  })
