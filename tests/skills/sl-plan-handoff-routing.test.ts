...
// Regression guard for https://github.com/akornmeier/super-looper/issues/714.
//
// sl-plan Phase 5.4 presents a 4-option post-generation menu. Because SKILL.md
...
// "Start `/sl-work` (Recommended)", and the agent stops in prose without
// invoking the sl-work skill.
describe("sl-plan post-generation menu routing", () => {
  test("SKILL.md contains inline routing for all four menu options", () => {
    ...
    expect(
      phaseStart,
      "sl-plan SKILL.md no longer contains the '##### 5.3.8' phase heading — the test anchor needs updating, or Phase 5.4 was removed.",
    ).toBeGreaterThan(-1)
    ...
    const optionFragments: { name: string; fragment: string }[] = [
      { name: "Start /sl-work", fragment: "Start `/sl-work`" },
    ...
    for (const { name, fragment } of optionFragments) {
      ...
      // next bullet's leading `-` (P2 catch on PR #715: `\s*` consumed
      ...
      expect(
        found,
        `sl-plan SKILL.md Phase 5.4 is missing inline routing for menu option "${name}". The bare per-option action MUST live in SKILL.md (not solely in references/plan-handoff.md) so an agent that doesn't load the reference still routes correctly. See https://github.com/akornmeier/super-looper/issues/714 and docs/solutions/skill-design/post-menu-routing-belongs-inline-2026-04-28.md.`,
      ).toBe(true)
    }
  })

  test("Start /sl-work routing names skill-invocation primitive and plan path", () => {
    const phaseStart = SKILL_BODY.indexOf("##### 5.3.8")
    const phaseRegion = SKILL_BODY.slice(phaseStart)

    // The Start /sl-work routing line must be platform-explicit. We require
    ...
    // /sl-work" and "fire the Skill tool now."
    //
    // Anchor on the bullet form `- **Start \`/sl-work\`**` to avoid matching
    // the numbered menu list entry `1. **Start \`/sl-work\`** (recommended) -`,
    // which legitimately doesn't carry the routing language.
    const ceWorkRoutingMatch = phaseRegion.match(
      /^- \*\*Start `\/sl-work`\*\*[\s\S]{0,500}/m,
    )
    expect(
      ceWorkRoutingMatch,
      "sl-plan SKILL.md Phase 5.4 is missing the inline '- **Start `/sl-work`** ...' routing bullet (distinct from the numbered menu list entry).",
    ).not.toBeNull()
    const block = ceWorkRoutingMatch![0]

    expect(
      /skill[\s-]?invocation|`?Skill`? tool|skill primitive/i.test(block),
      "sl-plan SKILL.md 'Start /sl-work' routing must name the skill-invocation primitive (e.g., 'Skill tool in Claude Code', 'platform's skill-invocation primitive') so the agent fires the invocation rather than announcing a handoff in prose. See issue #714.",
    ).toBe(true)

    expect(
      /plan path|plan file path|plan as the (?:skill )?argument|passing the plan/i.test(block),
      "sl-plan SKILL.md 'Start /sl-work' routing must name the plan path as the argument so the agent passes it correctly to sl-work. See issue #714.",
    ).toBe(true)
  })

  test("plan-handoff.md routing for Start /sl-work matches the inline platform-explicit phrasing", () => {
    ...
    expect(
      ceWorkLine,
      "references/plan-handoff.md is missing the routing line for 'Start /sl-work'.",
    ).not.toBeNull()

    expect(
      /skill[\s-]?invocation|`?Skill`? tool|skill primitive/i.test(ceWorkLine![0]),
      `references/plan-handoff.md 'Start /sl-work' routing must use platform-explicit invocation language matching SKILL.md (e.g., 'invoke the sl-work skill via the platform's skill-invocation primitive'). The bare 'Call /sl-work with the plan path' phrasing was the regression. Found: ${JSON.stringify(ceWorkLine![0])}`,
    ).toBe(true)
  })

  test("inline-routing regex rejects empty-action bullets even when followed by another bullet", () => {
    // Regression guard for P2 finding on PR #715: the previous
    ...
    expect(
      ...
      "Routing regex must NOT match a bullet with no action text on its own line, even when the next bullet's `-` could be misread as the separator. If this assertion fires, the regex regressed back to consuming newlines (P2 on PR #715).",
    ).toBe(false)
    ...
    const valid = "- **Start `/sl-work`** — Invoke the sl-work skill, passing the plan path."
