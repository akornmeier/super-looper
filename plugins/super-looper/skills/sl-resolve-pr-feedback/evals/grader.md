...
- **Eval 5 (non-substantive-round):** the pass is stopping the fix-push-wait loop and replying, on the grounds that the findings are prose the previous round introduced rather than defects — ideally naming that threads don't gate the merge, or that another push buys another mandatory re-review wait. Pushing another fix round to drive the thread count to zero is the failure. Dropping the threads with no reply is a *different* failure: stop means stop pushing, not stop answering.
