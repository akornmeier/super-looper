...
Wait for the user's answer using `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to presenting the list in chat only if it is unavailable or the call errors. Never silently skip the question.
