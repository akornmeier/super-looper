...
const AGENT_PATH = path.join(
  process.cwd(),
  "plugins/super-looper/agents/sl-session-historian.md",
)
const AGENT_BODY = readFileSync(AGENT_PATH, "utf8")

// Regression guard for https://github.com/akornmeier/super-looper/issues/794.
//
// `sl-session-historian` runs in subagent context (dispatched by `sl-sessions`
// and historically by `sl-compound` Phase 1). Claude Code does not permit
...
//
// The fix moved all script orchestration into the `sl-sessions` skill
...
describe("sl-session-historian no-Skill-tool regression guard", () => {
  test("agent body does not instruct Skill(sl-session-inventory) calls", () => {
    expect(AGENT_BODY).not.toMatch(/Skill\(\s*["'`]?sl-session-inventory/)
  })

  test("agent body does not instruct Skill(sl-session-extract) calls", () => {
    expect(AGENT_BODY).not.toMatch(/Skill\(\s*["'`]?sl-session-extract/)
  })
