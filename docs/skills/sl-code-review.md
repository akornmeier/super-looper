...
`sl-code-review` runs review as a structured pipeline with explicit gates:
...
- **Caller-owned apply + Residual Work Gate** — in `mode:agent` the caller (e.g. `/sl-work`) applies fixes and runs the Residual Work Gate (accept / file tickets / continue / stop); in interactive mode the review commits its applied fixes on a clean tree, and it never pushes
...
- **Always-on (every review)** — `sl-correctness-reviewer`, `sl-testing-reviewer`, `sl-maintainability-reviewer`, `sl-project-standards-reviewer`, `sl-agent-native-reviewer`, `sl-learnings-researcher`
...
- **CE conditional (migrations)** — `sl-deployment-verification-agent` for risky migration diffs; schema drift and migration safety are handled by the `data-migration` persona
...
| **`mode:agent`** | `mode:agent` (alias `mode:headless`) | One JSON object; report-only — the review mutates nothing and the caller (e.g. `/sl-work`) applies findings and owns the Residual Work Gate |
...
When autofix mode runs and the in-skill fixer can't resolve everything, the residual work doesn't just disappear into chat. The Residual Actionable Work summary lists each unresolved finding with stable numbering, severity, file:line, title, and autofix class. Callers (e.g., `/sl-work` Phase 3.4) read this summary and present user options: apply now, file tickets, accept with durable sink, or stop.

### 8. Protected artifacts

Super-looper pipeline artifacts (`docs/brainstorms/*`, `docs/plans/*.md`, `docs/solutions/*.md`) are protected — reviewers' findings to delete or gitignore them are discarded during synthesis. These are decision artifacts the pipeline depends on; reviewers shouldn't garbage-collect them.

---

## Quick Example

You invoke `/sl-code-review` on a feature branch with a Rails auth change that includes a database migration.
...
- Another skill is escalating to it (`/sl-work` Phase 3.3 Tier 2, `/sl-optimize` Phase 4.3)

Skip `sl-code-review` when:
...
- You want to fix bugs you find, not review code → use `/sl-debug`

---

## Use as Part of the Workflow

`sl-code-review` is invoked from multiple skills as the deep-review path:

- **`/sl-work` Phase 3.3** — escalates to `sl-code-review mode:agent` for sensitive surfaces, ≥400 lines + diffuse, ≥1,000 lines, or explicit thorough-review requests; sl-work then applies the findings
- **`/sl-work` Phase 3.4 Residual Work Gate** — reads the Residual Actionable Work summary `sl-code-review` returned and presents user options
- **`/sl-optimize` Phase 4.3** — runs against the cumulative optimization branch diff before merging
- **`/sl-doc-review`** — sibling skill for docs (requirements, plans), not code

Tier 1 (harness-native `/review`) handles most cases; `sl-code-review` is the Tier 2 escalation.
...
- **Current branch** — `/sl-code-review`
- **Specific PR** — `/sl-code-review 1234` or `/sl-code-review <PR URL>`
- **Specific branch** — `/sl-code-review feat/notification-mute`
- **With base ref** — `/sl-code-review base:abc1234` or `base:origin/main` (skips scope detection; reviews against that ref)
- **With plan** — `/sl-code-review plan:docs/plans/.../plan.md` for explicit requirements verification
...
No — the skill is tightly coupled to git, code reviewers, and PR contexts. For docs (requirements, plans), use `/sl-doc-review` instead.

---

## See Also

- [`sl-work`](./sl-work.md) — primary upstream caller; escalates to `sl-code-review` at Phase 3.3
- [`sl-doc-review`](./sl-doc-review.md) — sibling skill for documents (requirements, plans), not code
- [`sl-debug`](./sl-debug.md) — for fixing bugs found during review, when root-cause investigation matters
- [`sl-resolve-pr-feedback`](./sl-resolve-pr-feedback.md) — handles incoming reviewer comments after a PR is open
- [`sl-simplify-code`](./sl-simplify-code.md) — invoked by `sl-work` before review; complement, not substitute
