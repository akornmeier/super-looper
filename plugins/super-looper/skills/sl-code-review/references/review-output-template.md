...
- **Learnings & Past Solutions section** -- results from sl-learnings-researcher, with links to docs/solutions/ files
- **Agent-Native Gaps section** -- results from sl-agent-native-reviewer. Omit if no gaps found.
- **Deployment Notes section** -- key checklist items from sl-deployment-verification-agent. Omit if the agent did not run. Schema drift surfaces as `data-migration` findings — no separate section.
...
When `mode:agent` is active, **do not** emit the markdown table report above. Emit **one parseable JSON object** as the primary response and write the same payload to `review.json` under `/tmp/super-looper/sl-code-review/<run-id>/`.
