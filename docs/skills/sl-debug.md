...
`sl-debug` runs investigation as a structured process with explicit gates:
...
`sl-debug` does not propose a fix until it can explain the full causal chain from trigger to symptom with no gaps. "Somehow X leads to Y" is a gap. The fix gate is structural: there's an explicit phase transition that requires the chain explanation to pass.
...
- Hypotheses point to different subsystems → likely architecture problem; suggest `/sl-brainstorm`
...
Concrete signals trigger a `/sl-brainstorm` recommendation rather than a fix: the root cause is a wrong responsibility or interface; the requirements are wrong or incomplete; every fix is a workaround. Size alone doesn't make something a design problem — clear-fix-but-large bugs are still bugs.
...
You pick "fix it now." It creates a feature branch, writes the failing test, verifies it fails for the right reason, implements the minimal fix, runs tests, and hands off to `/sl-commit-push-pr`.

---

## When to Reach For It

Reach for `sl-debug` when:
...
- You already know the root cause and the fix is obvious — just fix it (or use `/sl-work` for a small change)
- The "bug" is really a feature decision in disguise → `/sl-brainstorm`
- The work is implementing something new, not investigating something broken → `/sl-work`

---

## Use as Part of the Workflow

`sl-debug` interlocks with the rest of the chain in three ways:

- **Called from `/sl-plan`** — when a planning prompt is bug-shaped (error message, "fix the bug where X", regression), `sl-plan` surfaces `sl-debug` as a route-out option before doing structural planning
- **Escalates to `/sl-brainstorm`** — when investigation reveals a design problem rather than a logic error, the skill recommends rethinking before implementing
- **Hands off to `/sl-commit-push-pr`** — after a successful fix on a skill-created branch, the skill defaults to commit-and-PR without further prompting (with an explicit override path if your repo's `AGENTS.md` says otherwise)

After a PR opens, the skill optionally offers `/sl-compound` to capture learning — but only when the bug is generalizable (3+ recurrence, wrong assumption about a shared dependency). Localized mechanical fixes are skipped silently to avoid cluttering `docs/solutions/` with one-off entries.

---

## Use Standalone

`sl-debug` is the standalone entry point for most bug work:

- **Failing test** — `/sl-debug spec/models/notification_subscription_spec.rb`
- **Error message paste** — `/sl-debug` followed by a stack trace
- **GitHub issue** — `/sl-debug #1234` or `/sl-debug https://github.com/.../issues/1234`
- **Linear ticket** — `/sl-debug ABC-456` or paste the URL
- **Stuck on something** — `/sl-debug "why is X returning undefined when Y"`
...
**When should the skill suggest `/sl-brainstorm`?**
...
Skip the skill — go directly to `/sl-work` or just edit the file. `sl-debug` is for cases where the root cause isn't obvious or the fix has failed to stick.
...
- [`sl-plan`](./sl-plan.md) — routes bug-shaped prompts here when you start at planning
- [`sl-brainstorm`](./sl-brainstorm.md) — escalation target when the bug reveals a design problem
- [`sl-work`](./sl-work.md) — sibling skill for feature work; use this when input isn't bug-shaped
- [`sl-commit-push-pr`](./sl-commit-push-pr.md) — handles the final commit + PR after a fix
- [`sl-compound`](./sl-compound.md) — capture reusable learning when the bug is generalizable
