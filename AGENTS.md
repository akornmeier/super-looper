...
- **Merge policy:** Pull requests are the required path for every change to `main`, and the ruleset now binds everyone — the bypass actor was removed, so the maintainer cannot push directly to `main` or merge past a red gate either. A direct push is refused with "Changes must be made through a pull request." The enforcement contract — a pull-request rule plus the `test`, `pr-title`, `pr-size`, and `pr-reviewed` status checks — is specified in `docs/solutions/workflow/main-ruleset-contract.md`. Verify the live state with `gh api repos/akornmeier/super-looper/rulesets/17763543` rather than trusting this file's prose. If a direct push ever did land (it can't now), it would be gated too late: `ci.yml` runs on `push` to `main`, so `release:validate` and the suite execute after the commit is already on `main`, and the pull-request-only checks never run at all — the version drift this prevents is why past direct merges needed multi-PR recovery (see `docs/solutions/workflow/release-please-version-drift-recovery.md`). Emergencies disable ruleset enforcement (which is audited); they never restore a bypass. Read the contract doc before renaming a workflow job, since a required context is matched by literal name and a rename blocks every pull request.
- **Safety:** Do not delete or overwrite user data. Avoid destructive commands.
- **Goal-change protocol:** Autopilot — `lfg` and any skill running in unattended mode — never edits `STRATEGY.md` or the active plan document mid-run. Goal changes route through interactive `sl-strategy` or a human-approved plan revision before a new run. The Loop driver's checksum guard (exit 8, `typed_failure: "goal-drift"`) and the plugin's PreToolUse goal-guard hook enforce this; see the [Goal guard](CONCEPTS.md#goal-guard) concept.
