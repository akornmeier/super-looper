...
CRITICAL: You MUST execute every step below IN ORDER. Do NOT skip any required step. Do NOT jump ahead to coding or implementation. A verified written plan MUST exist BEFORE any work begins — produced by the plan phase (step 1) in description mode, or supplied via a `plan:<path>` marker and gated in plan-input mode (step 1 handles both). Violating this order produces bad output. **The one carve-out is resume mode** (a `resume:<path>` marker — see the Resume protocol below): there you re-verify the postconditions of the already-completed steps instead of re-executing them, then continue IN ORDER from the first not-yet-completed step. Resume is postcondition re-verification only — never a license to skip a gate on a step you actually run.
...
**Resume protocol.** A `resume:<path>` marker (loop.sh adds it only on a headless retry it chose to resume rather than cold-restart) names the SAME run-progress file, now carrying a prior attempt's recorded state. It is a third input mode alongside description and plan-input, and it is the CRITICAL header's one carve-out. When a `resume:<path>` marker is present:

1. Read `<path>`. It records `step` (the **last completed** step), `plan_path`, `branch`, `base_ref`, and `head_sha` from the crashed attempt.
2. **Re-verify the postconditions of steps 1..`step` cheaply — do NOT re-execute them.** These are read-only checks: the recorded `branch` is checked out and exists; in plan-input mode the step-1 plan gate passes on the recorded `plan_path`; the expected commits are present (the recorded `head_sha` is reachable in the branch history). Do not re-plan, re-run work, or re-review completed steps.
3. **On success, continue the pipeline at step `step`+1** — the first not-yet-completed step — IN ORDER, honoring every gate from there on, and keep writing progress at each subsequent boundary.
4. **On ANY postcondition mismatch, do NOT fake completion.** Record the discrepancy in the progress file (which step's postcondition failed, and how) and exit WITHOUT emitting `<promise>DONE</promise>`. loop.sh's next retry re-validates the file and cold-restarts honestly. A resumed run that cannot prove its prior steps is a crash, not a finish — never emit DONE to paper over it.
...
2. Invoke the `sl-work` skill.
   ...
   - **Plan-input mode:** invoke `sl-work` with the recorded plan path plus `mode:legacy-workflow mode:unattended` so the explicit compatibility route executes the old workflow and suppresses its interactive clarifying and branch-choice prompts under automation.
   - **Description mode:** invoke `sl-work` with `mode:legacy-workflow mode:unattended`; it picks up the plan `sl-plan` just wrote.
...
9. **CI watch and autofix loop** (only when an open PR exists for the current branch)
   ...
   3. Read the failure logs, identify the root cause, and apply a fix in the working tree. Do NOT weaken, skip, or mock the failing assertion to make it pass — repair the actual issue. If a *specific* failing check is a flaky test that has no fix path, **record a flaky-no-fix-path disposition for this run, tagged to that check** — carry these per-check dispositions in your run context through the remaining iterations and the give-up GATE — and document it as the residual outcome below rather than retrying without a code change. A flaky check contributes no code change of its own, but step (4) is per-iteration, not per-check: still commit and push any fix made for *other* checks in this iteration, and skip step (4) only when the iteration produced no code changes at all. Tag each check individually: a run with several failing checks may have a flaky one alongside a genuinely-unfixable one, and the give-up GATE reads these per-check dispositions to decide whether to escalate (see the debug-escalation rung).
   ...
   **Debug-escalation rung** (runs once; not a loop — no attempt counter, no return-to-iteration edge):
   ...
   1. The tree is clean here — the 3 iterations committed their fixes — so any working-tree change after the pass is attributable to the debug pass. Load the `sl-debug` skill with `mode:unattended`, passing the live failing state already in hand — for **each** remaining non-flaky failing check, its check name, its link, and its `gh run view --log-failed` output (this log content is the problem statement), so the pass has complete input when more than one genuine failure remains. The links are CI run URLs, not issue references, so `sl-debug` must root-cause from the log content and not fetch a URL as an issue. (There is no pre-extracted test path — it is derivable from the logs.) Restate for the pass: **do NOT weaken, skip, or mock the failing assertion to make it pass — repair the actual issue** — a masked failure shipped as green is strictly worse than the honest floor.
