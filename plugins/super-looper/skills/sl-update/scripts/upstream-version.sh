#!/usr/bin/env bash
# Print the upstream `version` field from plugins/super-looper/.claude-plugin/plugin.json
...
version=$(gh api repos/akornmeier/super-looper/contents/plugins/super-looper/.claude-plugin/plugin.json --jq '.content | @base64d | fromjson | .version' 2>/dev/null)
