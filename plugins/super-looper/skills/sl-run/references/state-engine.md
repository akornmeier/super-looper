...
## Serial kernel operations

```text
<kernel> init --kernel --target <repo-root> --plan <repo-relative-plan> [--strategy <repo-relative-path>] [--run-id <id>] [--state <absolute-state-path>] [--base-ref <ref>] [--max-repair-attempts 0..3]
...
`init --kernel` parses the canonical Markdown into `execution-plan.json`, hashes the raw plan and strategy, records branch/base/head, creates the serial workflow journal, and atomically writes `run-state.json`. The default state path is `/tmp/super-looper/sl-run/<run-id>/run-state.json`.
...
Agent and verifier returns enter through uniquely named `incoming-agent-*.json` and `incoming-verifier-*.json` staging files. Names beginning `agent-result-` or `verifier-result-` are kernel-owned immutable outputs and are rejected as inputs.
