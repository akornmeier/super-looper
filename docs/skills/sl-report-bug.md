...
| Question | Answer |
|----------|--------|
| What does it do? | Gathers structured bug info via 6 questions, collects environment data automatically, files a GitHub issue at `akornmeier/super-looper` |
| When to use it | When something in the super-looper plugin doesn't work and you want to report it |
...
`sl-report-bug` runs reporting as a structured intake → format → file flow:
...
- `uname -a` for OS info
- Reads the plugin manifest from `~/.claude/plugins/installed_plugins.json`
- Runs `claude --version`
...
`disable-model-invocation: true` prevents the skill from auto-firing on prose mentions of bugs. Bug reporting is a deliberate user choice — invoke `/sl-report-bug` directly.

---

## Quick Example

You hit a bug where `/sl-plan` produces a plan with U-IDs that aren't sequential. You invoke `/sl-report-bug`.
...
5. **Repro**: "Run `/sl-plan` from a brainstorm doc with 4 implementation units. The third unit gets numbered U4 instead of U3."
...
Formatted report goes to `gh issue create --repo akornmeier/super-looper --title "[super-looper] Bug: U-ID numbering skips U3 in initial plan generation" --body "..." --label "bug,super-looper"`.
...
Issue: https://github.com/akornmeier/super-looper/issues/812
Title: [super-looper] Bug: U-ID numbering skips U3 in initial plan generation
...
Reach for `sl-report-bug` when:

- A skill, command, agent, or MCP integration in super-looper doesn't work as expected
...
Skip `sl-report-bug` when:

- The bug is in a different plugin or tool (this filing target is hardcoded to super-looper)
- It's a feature request, not a bug → file a discussion or feature-request issue manually
- You're not sure if it's a bug or expected — check `/sl-release-notes` first to see if behavior changed in a recent release

---

## Use as Part of the Workflow

`sl-report-bug` is a standalone utility — doesn't sit inside the chain. It's invoked when something goes wrong and the user wants the maintainer to know.

Common companion skills:

- **`/sl-update`** — check version first; you might be reporting a bug that's already fixed in a newer version
- **`/sl-release-notes`** — check whether the behavior changed recently; might be intended
...
- `/sl-report-bug` — walks through the 6 questions
- `/sl-report-bug "brief description"` — uses the description as initial context; still walks through the structured questions for completeness
...
Repo target: `akornmeier/super-looper`. Title prefix: `[super-looper]`. Labels: `bug,super-looper` (with fallback to no labels if missing).
...
**Can I report a non-super-looper bug?**
This skill specifically files at `akornmeier/super-looper`. For other plugins or tools, file directly in their respective repos. The structure of this skill is generalizable, but the repo target is hardcoded.
...
- [`/sl-update`](./sl-update.md) — check plugin version; older versions may have fixed bugs
- [`/sl-release-notes`](./sl-release-notes.md) — check whether the behavior changed in a recent release; might not be a bug
