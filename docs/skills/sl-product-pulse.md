...
| When to use it | "Run a pulse", weekly recap, launch-day check, "how are we doing", `/sl-product-pulse 7d` |
| What it produces | A report saved to `docs/pulse-reports/YYYY-MM-DD_HH-MM.md`; key points surfaced in chat |
| What's next | Surface follow-ups to `/sl-ideate` or `/sl-brainstorm`; investigate specific issues with the native tools |
...
`sl-product-pulse` runs as a structured observation pass with explicit invariants:
...
The skill never mutates the product, the database, or any external system. The only writes are pulse settings appended to `.super-looper/config.local.yaml` (gitignored, machine-local) and the report file (`docs/pulse-reports/...`). MCP and other data-source tools are invoked read-only; if a tool offers write modes, they're not used.
...
It's Monday morning. You want to see how things went over the weekend. You run `/sl-product-pulse 72h`.

The skill detects this is a configured project (`pulse_product_name` is set in `.super-looper/config.local.yaml`), so it skips the interview and goes straight to Phase 2. It applies the 15-minute trailing buffer: `[Friday 5:45pm — Monday 8:45am]`.
...
The report writes to `docs/pulse-reports/2026-05-04_08-45.md`. The Headlines and top Followup surface in chat. You see the followup, decide to investigate the climbing error pattern with `/sl-debug`.

---

## When to Reach For It

Reach for `sl-product-pulse` when:
...
`sl-product-pulse` closes the outer feedback loop:
...
- `STRATEGY.md` (from `/sl-strategy`) seeds the metrics that get measured
- `/sl-product-pulse` produces the report and surfaces follow-ups
- Follow-ups feed back into `/sl-ideate` (what's worth exploring), `/sl-debug` (what's broken), or `/sl-brainstorm` (what to build next)
...
- **Default 24h** — `/sl-product-pulse`
- **Specific window** — `/sl-product-pulse 7d`, `/sl-product-pulse 1h` (launch check), `/sl-product-pulse 30d`
- **Reconfigure** — `/sl-product-pulse setup` (or `reconfigure`, `edit config`) re-runs the interview
- **First run** — `/sl-product-pulse` with no config triggers the setup interview, then the pulse
...
Configuration lives in `.super-looper/config.local.yaml` (gitignored, machine-local) under `pulse_*` keys: product name, primary event, value event, completion events, quality scoring, quality dimension, analytics source, tracing source, payments source, DB enabled, per-metric source overrides, pending metrics, excluded metrics, default lookback.
...
- [`sl-strategy`](./sl-strategy.md) — seeds the metrics that the pulse measures
- [`sl-ideate`](./sl-ideate.md) — common follow-up destination for surfaced signals
- [`sl-debug`](./sl-debug.md) — common follow-up destination for error patterns the pulse surfaces
- [`sl-brainstorm`](./sl-brainstorm.md) — when a pulse follow-up needs scope clarification before fixing
