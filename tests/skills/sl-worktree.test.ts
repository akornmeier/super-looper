...
const SKILL_DIR = path.join(
  process.cwd(),
  "plugins/super-looper/skills/sl-worktree",
)
const SKILL_BODY = readFileSync(path.join(SKILL_DIR, "SKILL.md"), "utf8")

describe("sl-worktree SKILL.md", () => {
  // Issue #946: sl-worktree was re-architected from a script-based creator into
  ...
  test("ships no bundled script and no ${CLAUDE_SKILL_DIR} dependence", () => {
    expect(
      ...
      "sl-worktree must not bundle a scripts/ directory — it is now a portable inline-git guardrail (issue #946). A bundled script reintroduces the cross-platform path-resolution bug class (#943/#764).",
    ).toBe(false)
    expect(
      SKILL_BODY.includes("CLAUDE_SKILL_DIR"),
      "sl-worktree/SKILL.md must not reference ${CLAUDE_SKILL_DIR} — the guardrail uses inline git only, so it resolves on every platform without a skill-dir variable.",
    ).toBe(false)
    expect(
      SKILL_BODY.includes("worktree-manager.sh"),
      "sl-worktree/SKILL.md must not reference the removed worktree-manager.sh script.",
    ).toBe(false)
  })
  ...
  test("is portable: no ce_platforms gate", () => {
    ...
    expect(frontmatter, "sl-worktree/SKILL.md must have YAML frontmatter").not.toBeNull()
    expect(
      /^ce_platforms:/m.test(frontmatter![1]),
      "sl-worktree/SKILL.md must not declare `ce_platforms` — the inline-git guardrail is portable to all targets (issue #946).",
    ).toBe(false)
  })
  ...
  test("detects existing isolation before creating a worktree", () => {
    expect(
      ...
      "sl-worktree/SKILL.md must compare against --git-common-dir to detect an existing linked worktree (Step 0).",
    ).toBe(true)
    ...
    expect(
      ...
      "sl-worktree/SKILL.md must compare the resolved absolute git dir (`--absolute-git-dir`) so a subdirectory CWD in a normal checkout is not misread as an existing worktree.",
    ).toBe(true)
    expect(
      SKILL_BODY.includes("git rev-parse --show-superproject-working-tree"),
      "sl-worktree/SKILL.md must include the submodule guard (--show-superproject-working-tree) so a submodule is not mistaken for a worktree.",
    ).toBe(true)
    expect(
      /work in place/i.test(SKILL_BODY),
      "sl-worktree/SKILL.md must instruct the agent to work in place when already isolated, rather than nesting a worktree.",
    ).toBe(true)
  })
  ...
  test("prefers the harness's native worktree tool before the git fallback", () => {
    expect(
      ...
      "sl-worktree/SKILL.md must instruct the agent to prefer the harness's native worktree tool before falling back to git (avoids phantom state).",
    ).toBe(true)
  })
  ...
  test("documents an inline git fallback under .worktrees with gitignore safety", () => {
    expect(
      ...
      "sl-worktree/SKILL.md must document the inline `git worktree add` fallback.",
    ).toBe(true)
    expect(
      SKILL_BODY.includes("git check-ignore -q .worktrees/"),
      "sl-worktree/SKILL.md must probe `git check-ignore -q .worktrees/` WITH the trailing slash, so an existing directory-only `.worktrees/` ignore rule is honored and the skill doesn't redundantly dirty `.gitignore` (PR #948 review).",
    ).toBe(true)
  })
  ...
  test("anchors the git fallback at the repo root before using relative paths", () => {
    expect(
      ...
      "sl-worktree/SKILL.md must `cd \"$(git rev-parse --show-toplevel)\"` in the git fallback so relative `.worktrees`/`.gitignore` paths resolve at the repo root, not a subdirectory CWD.",
    ).toBe(true)
    expect(
      /run from the repo root/i.test(SKILL_BODY),
      "sl-worktree/SKILL.md Step 2 must explicitly instruct running the fallback from the repo root.",
    ).toBe(true)
  })
  ...
  test("treats the base-branch fetch as best-effort / non-fatal", () => {
    expect(
      ...
      "sl-worktree/SKILL.md must mark the `git fetch` step non-fatal so an absent `origin` remote falls through to the local base ref instead of aborting.",
    ).toBe(true)
  })
  ...
  test("on a sandbox/permission failure, asks rather than silently using the current checkout", () => {
    expect(
      ...
      "sl-worktree/SKILL.md must not instruct the agent to silently work in the current checkout on a sandbox failure — that defeats the isolation contract.",
    ).toBe(false)
    // The confirmation is blocking, so it must route through the
    // blocking-question tool (AskUserQuestion), not a vague "ask the user"
    // that can degrade to a non-blocking prompt.
    expect(
      SKILL_BODY.includes("AskUserQuestion"),
      "sl-worktree/SKILL.md must route the sandbox-failure confirmation through the blocking question tool (AskUserQuestion) rather than a non-blocking 'ask the user'.",
    ).toBe(true)
  })
})
