...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new sl-plan / sl-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- Requirements document → `sl-plan`
- Plan document → `sl-work`
...
- Do not modify caller skills (sl-brainstorm, sl-plan, or external plugin skills that invoke sl-doc-review)
