...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Transcription model to use, passed to the transcription endpoint configured via RIFFREC_TRANSCRIBE_URL and RIFFREC_TRANSCRIBE_API_KEY",
    )
...
def transcribe_media(media_path: Path | None, model: str) -> dict[str, Any]:
    ...
    api_key = os.environ.get("RIFFREC_TRANSCRIBE_API_KEY")
    if not api_key:
        return {
            "status": "skipped",
            "text": "",
            "reason": "RIFFREC_TRANSCRIBE_API_KEY is not set. Re-run with the key available to transcribe the media file.",
        }
    endpoint = os.environ.get("RIFFREC_TRANSCRIBE_URL")
    if not endpoint:
        return {
            "status": "skipped",
            "text": "",
            "reason": "RIFFREC_TRANSCRIBE_URL is not set. Point it at a Whisper-compatible /v1/audio/transcriptions endpoint to transcribe the media file.",
        }
...
def write_requirements_kickoff(
    ...
    lines.extend(
        [
            ...
            "- `sl-brainstorm` can confirm requirements from linked source evidence before any planning begins.",
            "",
            "---",
            "",
            "## Scope Boundaries",
            ...
            "- Brainstorm before plan: Use `sl-brainstorm` to refine product behavior when the recording reveals ambiguity.",
            "",
            "---",
            "",
            ...
            "## Next Steps",
            "",
            "-> Resume `/sl-brainstorm` to confirm candidate findings and replace generic R-items with product-specific requirements.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    print(f"Brainstorm handoff: $super-looper:sl-brainstorm {display_path(kickoff_md, repo_root)}")
