...
| run            | `/sl-run`           | Route canonical plans through code-owned checks, engineer review, approved delivery, CI repair, and evidence closeout                                        |
...
**Unattended (`scripts/loop.sh`).** For a fully hands-off run in a clean context, the loop driver launches `sl-run` for a canonical plan and preserves its durable state across process retries. Deterministic code selects a right-sized profile and owns checks and repair routing; bounded agents implement and independently verify. Hotfixes stop for proposal approval, and unattended runs stop at `review_ready`; interactive continuation requires final engineer approval before delivery and evidence closeout. Plan mode therefore requires a local `--verify-cmd`. It runs against **another** project, not this one:
