...
const SKILL_PATH = path.join(
  process.cwd(),
  "plugins/super-looper/skills/sl-brainstorm/SKILL.md",
)
...
const HTML_OUTPUT_PATH = path.join(
  process.cwd(),
  "plugins/super-looper/skills/sl-brainstorm/references/html-rendering.md",
)

// Mirror of the sl-plan output-mode tests. sl-brainstorm gains the same
...
// byte-for-byte from sl-plan (enforced by tests/compound-support-files.test.ts).
describe("sl-brainstorm output:html mode", () => {
  test("argument-hint advertises output:html", () => {
    ...
    expect(
      ...
      `sl-brainstorm argument-hint must mention 'output:html' so humans discover the flag. Current value: ${JSON.stringify(hint)}`,
    ).toBe(true)
  })
  ...
  test("SKILL.md describes the Output Mode resolution inline", () => {
    ...
    expect(
      phaseStart,
      "sl-brainstorm SKILL.md is missing the Phase 0.0 Output Mode resolution section.",
    ).toBeGreaterThan(-1)
    ...
    expect(
      ...
      "Phase 0.0 must name the `brainstorm_output` config key (the sl-brainstorm parallel to sl-plan's `plan_output`).",
    ).toBe(true)
    expect(
  ...
  test("config matching rule ignores commented YAML lines (active-key principle)", () => {
    // Parity with sl-plan side. Same review found that "contains
  ...
  test("unknown-value fallback note reflects final resolved mode, not a hardcoded md", () => {
    // Parity with sl-plan side. Hardcoding "defaulting to md" misleads users
  ...
  test("brainstorm-to-plan handoff does NOT auto-propagate output:", () => {
    // Asymmetric output is acceptable. sl-plan re-resolves its own
    ...
    expect(
      ...
      "sl-brainstorm SKILL.md must state that the output: preference does not auto-propagate to sl-plan on handoff (sl-plan re-resolves its own plan_output independently).",
    ).toBe(true)
  })
