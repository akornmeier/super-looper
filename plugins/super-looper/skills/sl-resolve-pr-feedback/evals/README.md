...
**Don't lead the witness.** Evals 5 and 6 are written so the agent has to do the classification the skill exists to teach. Their first drafts did not, and it cost the suite its signal: eval 5 told the agent *"Neither names a defect, and no test would change,"* and eval 6 spelled out the bug's mechanism and then named the bad excuses out loud (*"you are tired of the loop… unresolved threads do not block merging"*). Graded against a snapshot of the pre-stop-rule skill, both passed — a capable model reaches the right answer from the prompt alone, with no skill in the loop. The rewrite hands over only what the real flow hands over:

- **Raw reviewer text, no labels.** Eval 5's two threads are quoted as Copilot would write them; nothing in the scenario says they are prose-only. Eval 6 quotes the offending shell helper and the reviewer's claim about it, and never asserts the claim is correct — the agent must trace `$( … ; printf 'X')` and see for itself that `|| return 1` is unreachable.
- **Pressure as fact, not as editorial.** Eval 6 still supplies what would tempt a bad stop — two fix rounds already spent, the finding sitting on code the agent's own last round added — but states it flatly and leaves the rationalization unwritten.
- **Placed where a round-budget rule disagrees.** Eval 5 sits at the *second* fix cycle, where the old "first, second, or third cycle: repeat" rule would say to loop. A stop there can only come from reading the findings, which is the whole point.

The rewrite restored the signal, and it reversed the earlier conclusion. Eval 5 now discriminates cleanly — the current skill stops 3/3, the baseline pushes another commit 3/3 — and the baseline's failure is sharper than expected: all three baseline runs *correctly name* the pattern ("a self-feeding nit loop") and then push anyway, deferring the stop to a hypothetical next round. A fix-by-default bias overrides their own right judgment, and a stop that is always *next* round still pays this round's mandatory re-review wait. Eval 6 stays 6/6 across both arms, which is the correct result for a guard rail: it exists to fail loudly if the stop rule ever becomes cover for skipping a bug, not to separate the two skills.

**Scenario placement relative to the max-fix-round cap is load-bearing.** A draft of eval 6 put the fix at the *fourth* pass, past the cap of 3 — and measured the cap instead of the stop rule. The agents verified the defect, declined to auto-fix, and escalated it as `needs-human` with a ready diff, which is the cap working as designed. The two rules compose (the cap is gated on "if new **substantive** threads remain"), so an eval that wants the stop rule must sit *inside* the cap, and an eval that wants the cap should say so and be written separately.

The general lesson, for anyone writing the next eval here: if a competent model passes the scenario without the skill, the eval is measuring the model, not the skill. Test a rewrite against a snapshot of the skill *before* the behavior existed — if the baseline still passes, the prompt is doing the work.
