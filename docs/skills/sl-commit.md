...
`sl-commit` is the **commit-only** skill — sibling to `/sl-commit-push-pr` for the case where you want commits without pushing or opening a PR. It picks up your repo's existing commit conventions (project instructions first, then recent commit history, then conventional-commits as fallback), groups changed files by naturally distinct concerns (no `git add -p`, file level only), and avoids `git add -A` / `git add .` so sensitive files (`.env`, credentials, build artifacts) don't sneak in.

For the full ship flow (commit + push + PR), use `/sl-commit-push-pr`. For just the commit, this skill.
...
| What's next | `/sl-commit-push-pr` later if you want to open a PR; otherwise `git push` manually |
...
`sl-commit` runs commit creation as a structured pass:
...
You finish a feature touching backend models, controller, and frontend component. You invoke `/sl-commit`.
...
- You also want to push and open a PR → `/sl-commit-push-pr` does it all
...
`sl-commit` is invoked from skills that explicitly want commit-only flow:

- **`/sl-debug` Phase 4** — when the skill is on a pre-existing branch (not skill-owned) and the user picks "Commit the fix" instead of "Commit and PR", `sl-commit` handles the local commit
- **`/sl-work` Phase 4 (no-PR path)** — when the user prefers to commit without pushing, `sl-commit` is the alternate handoff
- Standalone via `/sl-commit`

The full ship flow (commit + push + PR) is `/sl-commit-push-pr`; this skill is the local-only sibling.
...
- `/sl-commit` — commit current changes following repo conventions
...
Use `/sl-commit-push-pr` for the full flow, or run `git push` and `gh pr create` manually after this skill commits.

---

## See Also

- [`/sl-commit-push-pr`](./sl-commit-push-pr.md) — full flow: commit + push + PR
- [`/sl-debug`](./sl-debug.md) — invokes this skill at Phase 4 for the commit-only handoff path
- [`/sl-work`](./sl-work.md) — invokes this skill at Phase 4 when the user picks no-PR
