...
`sl-test-browser` is the **end-to-end browser testing** skill. It maps changed files to testable routes, starts (or verifies) the dev server, navigates to each affected page via `agent-browser`, captures snapshots and screenshots, exercises critical interactions, pauses for human verification on flows that require external interaction (OAuth, email, payments, SMS), and produces a structured test summary. Headed mode lets you watch tests run; headless is faster and runs in the background.
...
`sl-test-browser` runs end-to-end tests as a structured flow:
...
When `agent-browser` isn't installed, the skill stops with `/sl-setup` as the install path — it doesn't try to fall back.
...
You finish a notification settings page and a layout change. You invoke `/sl-test-browser`.
...
Reach for `sl-test-browser` when:
...
Skip `sl-test-browser` when:

- The change is backend-only (no observable browser-visible behavior)
- `agent-browser` isn't installed → run `/sl-setup` first
...
`sl-test-browser` is invoked at the verification side of the chain:

- **`/sl-code-review` Tier 2** — for browser-affecting PRs, can spawn this skill to verify behavior in addition to static review
- **`/sl-work` Phase 3** — appropriate before opening the PR for UI-heavy work; the test summary becomes part of the PR description's verification narrative

`mode:report-only` for `sl-code-review` is the only review mode safe to run concurrently with this skill on the same checkout — other modes mutate, which would interfere with the running dev server's state.
...
- **Current branch** — `/sl-test-browser`
- **Specific PR** — `/sl-test-browser 847`
- **Specific branch** — `/sl-test-browser feature/new-dashboard`
- **Custom port** — `/sl-test-browser --port 5000`
- **Pipeline mode** — `/sl-test-browser mode:pipeline` (auto-starts server, scans for free port)
...
Required: `agent-browser` CLI installed (run `/sl-setup` if missing). Local dev server running (manual mode) or available start command (pipeline mode).
...
Predictable behavior across platforms and modes. Falling back to Chrome MCP or IDE built-ins means three tools' quirks instead of one. The skill is explicit about it: in Claude Code, do not use `mcp__claude-in-chrome__*` or substitute unrelated browsing tools.
...
**Can it run concurrent with `sl-code-review`?**
...
- [`sl-code-review`](./sl-code-review.md) — can spawn this skill for browser-affecting PRs (use `mode:report-only` for concurrent runs on the same checkout)
- [`sl-test-xcode`](./sl-test-xcode.md) — sibling skill for iOS simulator testing
- [`sl-demo-reel`](./sl-demo-reel.md) — captures visual evidence for PR descriptions; complementary to test summary
- [`sl-work`](./sl-work.md) — orchestrator that may invoke this skill during Phase 3 verification
