...
- **`sl-learn` (step 10)** sets `refresh_due` when a committed learning pushes the corpus past the refresh threshold; loop.sh's `emit_record` lifts it into the run-record verbatim (R13). The signal is advisory — `sl-learn` never dispatches `sl-compound-refresh`.
