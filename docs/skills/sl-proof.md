...
| When to use it | "Share to Proof", "view this in Proof", "HITL this doc with me", "iterate with Proof on this draft"; auto-invoked on `sl-brainstorm` / `sl-plan` / `sl-ideate` HITL handoffs |
...
`sl-proof` runs collaboration through Proof's structured API:
...
- **Consistent identity** — `by: "ai:super-looper"` on every op; `name: "Super Looper"` bound once via `/presence`
...
The skill documents both. Identity stays consistent: `ai:super-looper` machine ID, `Super Looper` display name. Callers running HITL review in different sub-agent contexts can override the identity pair if a distinct sub-agent should own the doc.
...
- **Upstream skill handoff** — `sl-brainstorm` / `sl-ideate` / `sl-plan` finishes a draft and passes it for review
...
The skill enforces `by: "ai:super-looper"` on every op and `X-Agent-Id: ai:super-looper` in headers. Display name `Super Looper` is bound once per session via `/presence`. **Don't use `ai:compound` or other ad-hoc variants** — identity stays uniform unless a caller explicitly overrides for a sub-agent context.

---

## Quick Example

`/sl-plan` finishes a notification-mute plan and the user picks "Open in Proof" at the Phase 5.4 menu. Plan invokes `sl-proof` in HITL-review mode with the plan path and title.
...
After all threads are processed, the skill asks the user to confirm the end-sync. User confirms. The skill atomically writes the reviewed markdown back to `docs/plans/2026-05-04-001-feat-notification-mute-plan.md`. Returns control to `sl-plan` Phase 5.4 with `status: proceeded` and `localSynced: true`.

---

## When to Reach For It

Reach for `sl-proof` when:
...
- A chain skill (`sl-brainstorm`, `sl-plan`, `sl-ideate`) handed off for human review
...
`sl-proof` integrates with the chain at multiple HITL touchpoints:

- **`/sl-brainstorm` Phase 4** — "Open in Proof" handoff for collaborative iteration on the requirements doc
- **`/sl-plan` Phase 5.4** — "Open in Proof" handoff for HITL review of the plan
- **`/sl-ideate` Phase 6** — "Open and iterate in Proof" option (default save destination for non-software topics)
- **`/sl-compound`** — for sharing a learning before committing to `docs/solutions/`
...
- **Upload local markdown** — `/sl-proof "share docs/plans/foo.md to Proof for iteration"`
- **From a Proof URL** — `/sl-proof https://www.proofeditor.ai/d/abc123?token=xxx` (read state, add comments, suggest edits)
...
Identity defaults: `by: "ai:super-looper"`, `X-Agent-Id: ai:super-looper`, `name: "Super Looper"`. `Idempotency-Key` recommended on every mutation and required when the contract says so.
...
**Why the `ai:super-looper` identity?**
For consistent attribution. Mark authorship in the rendered doc shows who edited; if the agent uses `ai:compound` one day and `ai:super-looper` the next, the audit trail looks fragmented. The skill enforces one identity unless a caller explicitly overrides.
...
- [`/sl-brainstorm`](./sl-brainstorm.md) — Phase 4 "Open in Proof" handoff
- [`/sl-plan`](./sl-plan.md) — Phase 5.4 "Open in Proof" handoff
- [`/sl-ideate`](./sl-ideate.md) — Phase 6 "Open and iterate in Proof" option
