...
| What's next | Open the PR via `/sl-commit-push-pr` |
...
`sl-simplify-code` runs three parallel reviewers, each focused on one dimension:
...
You've spent an hour writing a notification-mute feature. Before opening the PR, you invoke `/sl-simplify-code`.
...
Reach for `sl-simplify-code` when:
...
`sl-simplify-code` is called automatically by `/sl-work` Phase 3 when a diff is ≥30 changed lines — it runs before the harness-native or `/sl-code-review` review tier so reviewers see the simplified diff. It's also commonly invoked manually before `/sl-commit-push-pr`, when you want a refinement pass on a branch you've been building over multiple sessions.

The flow when manually invoked typically looks like:

```text
write code → /sl-simplify-code → /sl-commit-push-pr
```
...
- **Pre-PR refinement** — `/sl-simplify-code` on a feature branch before opening a PR
- **Post-AI cleanup** — when an LLM generated code that ships but feels over-engineered
- **Targeted refinement** — `/sl-simplify-code "the changes I made to NotificationDispatcher"` honors a user-named scope
- **Single-file pass** — `/sl-simplify-code app/services/notification_dispatcher.rb`
...
By default it runs against whatever scope it resolves, but the yield on tiny diffs (a couple of lines) is low. Inside `sl-work`, the skill is gated by the ≥30-line threshold for that reason.

---

## See Also

- [`sl-work`](./sl-work.md) — calls this skill in Phase 3 for diffs of significant size
- [`sl-commit-push-pr`](./sl-commit-push-pr.md) — usual next step after a refinement pass
- [`sl-code-review`](./sl-code-review.md) — the deeper code review skill; `sl-simplify-code` is a complement, not a substitute
