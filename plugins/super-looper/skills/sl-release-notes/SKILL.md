...
description: Summarize recent super-looper plugin releases, or answer a specific question about a past release with a version citation. Use when the user types `/sl-release-notes` or asks "what changed in super-looper recently?" or "what happened to `<skill-name>`?".
...
Look up what shipped in recent releases of the super-looper plugin. Bare invocation summarizes the last 5 plugin releases. Argument invocation searches the last 40 releases and answers a specific question, citing the release version that introduced the change.

Data comes from the GitHub Releases API for `akornmeier/super-looper`, filtered to the `super-looper-v*` tag prefix so sibling components (`cli-v*`, `coding-tutor-v*`, `marketplace-v*`) are excluded.
...
Version-like inputs (`2.65.0`, `v2.65.0`, `super-looper-v2.65.0`) are query strings, not a separate lookup-by-version mode. They flow through query mode like any other text.
...
> `python3` is required to run `/sl-release-notes`. Install Python 3.x and retry, or open https://github.com/akornmeier/super-looper/releases directly.
...
```json
{
  ...
  "fetched_at": "...",
  "releases": [
    {"tag": "super-looper-v2.67.0", "version": "2.67.0", "name": "...",
...
If `ok: true`, take the first 5 entries from `releases` (the helper has already filtered to `super-looper-v*` and sorted newest first). If fewer than 5 are available, render whatever count came back without warning.
...
```
Showing the last 5 releases. For older history, ask a specific question (e.g., `/sl-release-notes what happened to <skill>?`).
Browse all releases at https://github.com/akornmeier/super-looper/releases
```
...
```bash
gh pr view <linked_prs[0]> --repo akornmeier/super-looper --json title,body,url
```
