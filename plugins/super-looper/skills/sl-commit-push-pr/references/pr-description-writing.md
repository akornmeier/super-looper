...
- Bad: "Adds `evidence-decider.ts`, modifies `sl-commit-push-pr/SKILL.md` to call it, and updates two test files."
...
In order: opening → body sections that earn their keep → test plan if non-obvious → evidence block if one exists → Super Looper badge after a `---` rule.
...
[![Super Looper](https://img.shields.io/badge/Built_with-Compound_Engineering-6366f1)](https://github.com/akornmeier/super-looper)
