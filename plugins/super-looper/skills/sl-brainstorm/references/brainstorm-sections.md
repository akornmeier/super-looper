...
- **The planning agent** (`sl-plan` or a human) produces an implementation
...
- Any durable decisions made during the dialogue can flow naturally to
  downstream artifacts (`sl-plan`, the commit message, `docs/solutions/`)
...
decision flows to `sl-plan` (or directly to commit message, or to
...
- **Success Criteria** — include when there are quality / metric / handoff
  ...
  one voice"), process / handoff quality ("sl-doc-review can act on this
...
- **`topic`** — kebab-case slug identifying the brainstorm subject (e.g.,
  ...
  alongside `date` and as the resume-detection key when `sl-brainstorm`'s
...
(`sl-plan`, `sl-doc-review`) reference via the plan's `origin:` field. No
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  pointing to sl-plan, no italic provenance lines. Engineering process
