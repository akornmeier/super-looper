...
> Search and ask questions about your Claude Code session history.

`sl-sessions` is the **session-history search** skill. It's a thin user-facing entry point that dispatches `sl-session-historian` to search Claude Code session files for context relevant to your question — what you worked on, what was tried before, how a problem was investigated, what happened recently, decisions made.
...
| Question | Answer |
|----------|--------|
| What does it do? | Searches your Claude Code session history for context relevant to a question |
...
| Where it reads | Reads sessions from `~/.claude/projects/` |

---

## The Problem

Claude Code session history is ephemeral by default:
...
`sl-sessions` dispatches `sl-session-historian` — a specialized agent that reads Claude Code session files, applies time and repo filters, extracts findings relevant to the user's question, and synthesizes a structured digest:
...
The agent operates outside the working directory (in the session-file directory) which orchestrator-level tools often can't access — that's why this is delegated rather than handled inline.

---

## What Makes It Novel

### 1. Question-driven synthesis, not raw transcript dump
...
### 3. Thin orchestrator, agent does the work
...
You're picking up work on a feature you started two weeks ago. You can't quite remember whether you settled on per-subscription mute state or per-user mute state, and you want to confirm before continuing. You invoke `/sl-sessions "did we decide where notification mute state lives?"`.

The skill pre-resolves the branch (`tmchow/notification-mute`) and dispatches `sl-session-historian` with the question, current working directory, and branch hint.

The historian searches sessions on this branch over the last 30 days in `~/.claude/projects/`. It finds 5 Claude Code sessions. It reads through them looking for evidence on the mute-state question. It returns a digest:
...
Skip `sl-sessions` when:

- The context lives in committed code or docs, not in agent sessions → just read the code/docs
- You want general session metadata (count, timestamps, sizes) without semantic search → run `discover-sessions.sh` and `extract-metadata.py` from `plugins/super-looper/skills/sl-sessions/scripts/` directly
...
`sl-sessions` is mostly invoked standalone, but interlocks with other skills:

- **`/sl-compound` Phase 1 (Full mode)** — optionally invokes `sl-sessions` via the platform's skill-invocation primitive to search prior sessions for related context, folding findings into the new learning's "What Didn't Work" section
- **`/sl-debug` Triage** — prior-attempt awareness; when the user indicates failed attempts, asking "what have you already tried" before investigating avoids repeating known-failed approaches

This skill is the canonical entry point for Claude Code session search; other skills invoke it via the skill-invocation primitive when they need session-history context.
...
- **With a question** — `/sl-sessions "did we decide where notification mute state lives?"`
- **Without a question** — `/sl-sessions` asks "what would you like to know about your session history?"
...
**Where does it read sessions from?**
`sl-sessions` reads from `~/.claude/projects/` — your local Claude Code session files.
...
The user-facing surface should ask the right question if one wasn't given, and the orchestrator handles branch pre-resolution, scan-window choice, deep-dive selection, and per-session extraction in main context where script invocation works portably. The synthesis-only `sl-session-historian` subagent receives pre-extracted file paths and produces prose findings — it cannot run the discovery pipeline itself, by design.
...
- [`sl-compound`](./sl-compound.md) — invokes `sl-sessions` (opt-in) during Full-mode capture for prior-context enrichment
- [`sl-debug`](./sl-debug.md) — prior-attempt awareness uses similar context; ask the user about prior failed attempts when the signal is there
- `plugins/super-looper/skills/sl-sessions/scripts/` — the underlying scripts (`discover-sessions.sh`, `extract-metadata.py`, `extract-skeleton.py`, `extract-errors.py`) that sl-sessions invokes; can be run directly when raw metadata or extraction output is needed without orchestration
