...
**The honesty flag itself was fooled by non-emptiness (caught by the review bot on the PR).** Making a malformed fetch degrade to "nothing pending" is only half the honesty story — the gate also tracks whether any poll *actually reached the source*, so on a total fetch failure it reports "could not confirm quiescence" instead of a false "quiescent." That flag was first keyed on the payload being *non-empty* — which a `gh` error object or an HTML error page also satisfies. So a non-empty-but-wrong payload set the "fetch succeeded" flag, `pending` still came back empty, and the gate again claimed "quiescent" without ever confirming a re-review: the same false-success, one layer up. The lesson: **a non-empty response is not a successful fetch.** Gate the success flag on the payload's *shape* — `jq -e 'has("reviews")'`, "does it actually carry the field I asked for?" — not on mere non-emptiness, before trusting it as a real read. The recurring shape across all three of these is one rule: every signal the loop trusts (the re-review, the timeout bound, the fetch-succeeded flag) must be validated against what it actually proves, because each one has a cheap impostor (elapsed time, a non-empty error page) that passes a lazy check.

**The gate validated itself on its own PR.** Shipped in the same PR that introduced it, the gate then ran against that PR's review feedback. After one fix push it polled until the bot's review of the pushed commit appeared (`quiescent`), and the immediately-following re-fetch surfaced a *new* review thread an instant re-fetch would have missed — the exact async gap the pattern closes. On the next round it timed out (the bot had not re-reviewed within the bound) and honestly reported the still-pending bot rather than implying a quiescence it had not observed. Both halves of the contract — catch the async round when it lands, stay honest when it does not — held in practice, not just on paper.
