 ...
 *     into sl-compound and sl-sessions to derive a worktree-stable repo name.
 ...
 *   - sl-compound and sl-sessions used `git rev-parse ... | sed -E '...'` which
 ...
 *   - sl-compound and sl-sessions used `git rev-parse --abbrev-ref HEAD 2>/dev/null`
...
const PLUGIN_SKILLS_GLOB = ["plugins/super-looper/skills"]
...
describe("hasUnguardedErrorSuppression", () => {
  ...
  test("flags `command -v <name> 2>/dev/null` with no fallback", () => {
    expect(hasUnguardedErrorSuppression("command -v jq 2>/dev/null")).toBe(true)
  })
