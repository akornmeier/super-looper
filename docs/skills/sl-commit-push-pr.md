...
`sl-commit-push-pr` is a standalone Git utility for code already ready to ship. The primary `/sl-run` workflow performs authorized delivery through its code-owned kernel instead of invoking this skill.
...
`sl-commit-push-pr` handles each of those explicitly:
...
You finish a notification-mute feature on a feature branch. You invoke `/sl-commit-push-pr`.
...
It resolves the PR commit range, reads the diff over all commits (not just the working-tree diff), and detects the change has observable UI behavior. It asks whether to capture evidence; you say yes; it loads `/sl-demo-reel` and gets a GIF.
...
Reach for `sl-commit-push-pr` when:
...
Skip `sl-commit-push-pr` when:

- You want only commits without pushing or PR → `/sl-commit`
...
`sl-commit-push-pr` is the standard shipping handoff for several skills:

- **`/sl-work` Phase 4** — passes plan summary, key decisions, testing notes, evidence context, operational validation, and any accepted Known Residuals
- **`/sl-debug` Phase 4** (skill-owned branch) — defaults to commit-and-PR without prompting after a successful fix; includes auto-close syntax for the issue tracker (e.g., `Fixes #N` for GitHub, `Closes ABC-123` for Linear)
- **`/sl-compound`** — after a learning doc is written, can commit + push to update an open PR with the new commit
...
- **Full ship** — `/sl-commit-push-pr` from a feature branch with uncommitted or unpushed work
- **Refresh an existing PR's description** — `/sl-commit-push-pr "update the PR description"` or `/sl-commit-push-pr "include the benchmarking results"` (focus is honored)
- **Draft a description without applying** — `/sl-commit-push-pr "draft a PR description for this branch"` prints the description for you to copy or apply manually
- **Describe a different PR** — `/sl-commit-push-pr <PR URL>` resolves that PR's commit range
...
- [`sl-work`](./sl-work.md) — Phase 4 handoff target; standard upstream caller
- [`sl-debug`](./sl-debug.md) — calls this skill after a successful fix on a skill-owned branch
- [`sl-commit`](./sl-commit.md) — local-commit-only sibling; use when you don't want to push or open a PR
- [`sl-demo-reel`](./sl-demo-reel.md) — invoked for evidence capture when behavior is observable
- [`sl-compound`](./sl-compound.md) — capture reusable learning; can chain back into this skill to push the learning doc
