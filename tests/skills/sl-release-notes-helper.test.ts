...
async function makeGhShim(stdout: string, exitCode = 0): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "sl-rn-gh-"))
...
const PLUGIN_267 = {
  tagName: "super-looper-v2.67.0",
  name: "super-looper: v2.67.0",
  publishedAt: "2026-04-17T05:59:30Z",
  url: "https://github.com/akornmeier/super-looper/releases/tag/super-looper-v2.67.0",
  body:
    "## Features\n* **sl-polish-beta:** thing ([#568](https://github.com/akornmeier/super-looper/issues/568))\n* fixes ([#575](https://github.com/akornmeier/super-looper/issues/575))\n",
}

const PLUGIN_266 = {
  tagName: "super-looper-v2.66.1",
  name: "super-looper: v2.66.1",
  publishedAt: "2026-04-15T10:00:00Z",
  url: "https://github.com/akornmeier/super-looper/releases/tag/super-looper-v2.66.1",
  body:
    "## Bug Fixes\n* something ([#560](https://github.com/akornmeier/super-looper/issues/560))\n",
}
...
const CLI_267 = {
  ...
  url: "https://github.com/akornmeier/super-looper/releases/tag/cli-v2.67.0",
  body:
    "## Features\n* cli stuff ([#600](https://github.com/akornmeier/super-looper/issues/600))\n",
}
...
describe("list-plugin-releases.py", () => {
  describe("gh path", () => {
    test("mixed tags → only super-looper-v* surfaced, sorted newest first", async () => {
      ...
      expect(data.releases[0].tag).toBe("super-looper-v2.67.0")
      expect(data.releases[0].version).toBe("2.67.0")
      expect(data.releases[0].linked_prs).toEqual([568, 575])
      expect(data.releases[1].tag).toBe("super-looper-v2.66.1")
    })
    ...
    test("url prefers html_url over api url when both present", async () => {
      const apiShaped = {
        ...
        html_url:
          "https://github.com/akornmeier/super-looper/releases/tag/super-looper-v2.67.0",
        url:
          "https://api.github.com/repos/akornmeier/super-looper/releases/310187170",
  ...
  describe("anon error paths", () => {
    test("HTTP 403 + X-RateLimit-Remaining:0 → ok:false code=rate_limit", async () => {
      ...
      expect(data.error.user_hint).toContain(
        "github.com/akornmeier/super-looper/releases",
      )
  ...
  describe("integration", () => {
    test("invoked from an unrelated working directory still works", async () => {
      ...
      const tmpdir = await fs.mkdtemp(path.join(os.tmpdir(), "sl-rn-cwd-"))
      const env: Record<string, string> = {}
      for (const [k, v] of Object.entries(process.env)) {
        if (v !== undefined) env[k] = v
      }
