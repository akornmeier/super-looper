---
name: sl-sessions
description: "Search and ask questions about Claude Code session history. Use when asking what was worked on, what was tried before, how a problem was investigated across sessions, what happened recently, or any question about past agent sessions. Also use when the user references prior sessions, previous attempts, or past investigations — even without saying 'sessions' explicitly."
---

# /sl-sessions

Search Claude Code session history and synthesize findings about what was worked on, tried, decided, or learned in prior sessions.

## Usage

```
/sl-sessions [question or topic]
/sl-sessions
```
...
- **Never include thinking or reasoning block content.** Claude Code thinking blocks are internal reasoning and not actionable.
...
If no question argument is provided, ask what the user wants to know about their session history. Use `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to numbered options in chat only if it is unavailable. Never silently skip the question.
...
Each output line is a JSON object describing a session (platform, file, size, ts, session, branch). The final `_meta` line carries `files_processed` and `parse_errors`.
...
1. **Branch filter.** Keep sessions where `branch == dispatch_branch` exactly, or where the branch name contains a keyword from the question's topic (e.g., a question about "auth middleware" matches branches `feat/auth-fix`, `chore/auth-refactor`).

2. **If the branch filter returned zero sessions:**
...
5. **Apply the deep-dive cap.** Take at most **5 sessions total**. Narrow by branch-match → `match_count` → file size > 30KB → recency.
...
```bash
SCRATCH=$(mktemp -d -t sl-sessions-XXXXXX)
```
...
Dispatch the `sl-session-historian` subagent via the `Agent` tool. Omit the `mode` parameter so the user's configured permission settings apply. Run on the mid-tier model (e.g., `model: "sonnet"`) — the synthesizer doesn't need frontier reasoning.
...
- `output_schema` — the structure the agent's response should follow. Default schema:
  ...
  - Related context
  ```
  When the caller (e.g., `sl-compound`) supplies a schema in the skill argument, pass it through verbatim.
...
1. /tmp/sl-sessions-XXXX/abc123.skeleton.txt
   platform=claude branch=feat/auth-fix ts=2026-05-01
2. /tmp/sl-sessions-XXXX/def456.skeleton.txt  errors=/tmp/sl-sessions-XXXX/def456.errors.txt
   platform=claude branch=chore/auth-refactor ts=2026-05-03
...
...
When the caller (typically a user typing `/sl-sessions`, or another skill invoking sl-sessions via the platform's skill-invocation primitive) does not specify an output format, include a brief header noting what was searched:

```
**Sessions searched**: [count] Claude Code | [date range]
```
