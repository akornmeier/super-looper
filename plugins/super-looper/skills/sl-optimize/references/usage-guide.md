# `/sl-optimize` Usage Guide

## What This Skill Is For

`/sl-optimize` is for hard engineering problems where:
...
Use `/sl-optimize` when the problem looks like:
...
`/sl-optimize` is usually the wrong tool when:
...
```text
Use /sl-optimize to find the smallest memory setting that keeps this service stable under our load test.
...
```text
Use /sl-optimize to improve issue and PR clustering quality.
...
```text
Use /sl-optimize to create a summarization prompt for issues and PRs that minimizes token spend while still producing summaries that are good enough for downstream clustering.
