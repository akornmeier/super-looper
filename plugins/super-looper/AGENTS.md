...
These instructions apply when working under `plugins/super-looper/`.
They supplement the repo-root `AGENTS.md`.

# Super Looper Plugin Development
...
- When two or more skills share a behavioral principle, duplicate the guidance into each skill (inline for short rules, `references/` for longer ones). There is no cross-skill shared-file mechanism (see "File References in Skills" below). When a reference file is duplicated across skills (e.g., `concepts-vocabulary.md` in both `sl-compound/references/` and `sl-compound-refresh/references/`), edits must be applied to every copy in the same commit. Drift between copies produces inconsistent agent behavior depending on which skill loaded.
- Do not propose that runtime guidance for sl-ideate, sl-brainstorm, sl-plan, or any other skill live in this AGENTS.md or in the repo-root AGENTS.md. Those files only shape how contributors edit the plugin.
...
```
agents/
└── sl-*.md  # All agents live flat under agents/, prefixed with sl-

skills/
├── sl-*/          # Core workflow skills (sl-plan, sl-code-review, etc.)
└── */             # All other skills
```
...
> and work identically in Claude Code.
...
**All skills and agents** use the `sl-` prefix to unambiguously identify them as super-looper components:
- `/sl-brainstorm` - Explore requirements and approaches before planning
- `/sl-plan` - Create implementation plans
- `/sl-code-review` - Run comprehensive code reviews
- `/sl-work` - Execute work items systematically
- `/sl-compound` - Document solved problems

**Why `sl-`?** Claude Code has built-in `/plan` and `/review` commands. The `sl-` prefix (short for super-looper) makes it immediately clear these components belong to this plugin. The hyphen is used instead of a colon to avoid filesystem issues on Windows and to align directory names with frontmatter names.

**Agents** follow the same convention: `sl-adversarial-reviewer`, `sl-learnings-researcher`, etc. When referencing agents from skills, use the bare `sl-<agent-name>` form (e.g., `sl-adversarial-reviewer`) — the `sl-` prefix is sufficient for uniqueness across plugins.

**The `sl-` prefix is required for every new skill and agent — no exceptions.** Three legacy skills (`every-style-editor`, `file-todos`, `lfg`) predate the rule and remain unprefixed; they are pinned in `tests/frontmatter.test.ts` as the only allowed exceptions. Do not add to that allowlist. When adding a new skill, the directory name, the SKILL.md `name:` frontmatter, and any README references must all start with `sl-`. The frontmatter test enforces this and will fail on a missing prefix.

## Known External Limitations

**Proof HITL surfaces a ghost "AI collaborator" agent** (noted 2026-04-16, may change): The Proof API auto-joins any header-less `/state` read under a synthetic `ai:auto-<hash>` identity, so docs created by the `skills/proof/` HITL workflow show a phantom participant alongside `Super Looper`. The only way to suppress it is to set `ownerId: "agent:ai:super-looper"` on create — but that transfers document ownership to the agent and prevents the user from claiming it into their Proof library, so we don't use it. Treat as cosmetic noise; don't reintroduce the `ownerId` workaround. Tracked upstream: https://github.com/EveryInc/proof/issues/951.
...
- [ ] `description:` value is quoted (single or double) if it contains colons -- unquoted colons break `js-yaml` strict parsing. Run `bun test tests/frontmatter.test.ts` to verify.
...
- [ ] When a skill needs to ask the user a question, instruct use of `AskUserQuestion`.
- [ ] Also instruct to load `AskUserQuestion` via `ToolSearch` with `select:AskUserQuestion` first if its schema isn't already loaded — `AskUserQuestion` is a deferred tool and won't be available at session start. A pending schema load is not a valid reason to fall back to text.
- [ ] Include a fallback: when the call errors (e.g., `ToolSearch` returns no match), present numbered options in chat and wait for the user's reply — never silently skip the question.
...
> **Platform-behavior note (April 2026, may change):** The specifics above reflect current behavior — `AskUserQuestion` is deferred in Claude Code. If Anthropic changes `AskUserQuestion` to a non-deferred tool, revisit this guidance rather than carrying the workaround forward indefinitely. Verify before assuming these constraints still hold.

### Interactive Question Tool Design

Design rules for blocking question menus (`AskUserQuestion`). Violations silently degrade the UX where secondary description text is hidden or labels are truncated.

- [ ] Each option label must be self-contained — the harness may render only the label, not the accompanying description; the label alone must convey what the option does
- [ ] Keep total options to 4 or fewer (`AskUserQuestion` caps at 4)
...
- [ ] When a skill needs to create or track tasks, describe the intent (e.g., "create a task list") and use `TaskCreate`/`TaskUpdate`/`TaskList`
- [ ] Do not reference `TodoWrite` or `TodoRead` — these are legacy tools replaced by `TaskCreate`/`TaskUpdate`/`TaskList`

### Sub-Agent Dispatch

- [ ] When a skill dispatches sub-agents, use the `Agent`/`Task` tool
- [ ] Prefer bounded parallel execution: respect active-subagent limits, queue overflow work, and treat limit-related spawn errors as backpressure. Include a sequential fallback when parallel dispatch is unavailable
- [ ] Prefer sub-agents shipped with this plugin (`sl-*`) over built-ins. Exception: when a built-in (e.g., `Explore`) offers a meaningful benefit worth keeping, use it at the call site.

### Script Path References in Skills

- [ ] In bash code blocks, build co-located script paths from `${CLAUDE_SKILL_DIR}` — never a bare relative path and never `${CLAUDE_PLUGIN_ROOT}`. The runtime Bash tool runs from the user's project CWD, not the skill directory, so a bare `bash scripts/my-script` resolves against `<project>/scripts/` and fails — the recurring bug class behind #764, #811, and #898. `${CLAUDE_SKILL_DIR}` resolves the skill's own directory across both `claude --plugin-dir` and standard marketplace-cached installs.
- [ ] Reference the script with a backtick *read-time* path (e.g., `` `scripts/my-script` ``) so agents can locate it to read it — read-time references resolve against the skill directory; a markdown link is not needed since the bash code block already provides the invocation

### Skill Reference Rules

- [ ] When one skill refers to another skill, prefer semantic wording such as "load the `sl-doc-review` skill" rather than slash syntax.
- [ ] Use slash syntax only when referring to an actual published command or workflow such as `/sl-work` or `/sl-compound`.
...
- [ ] Use the native file-search/glob tool (`Glob`), content-search tool (`Grep`), and file-read tool (`Read`) instead
...
Plugin config lives at `.super-looper/config.local.yaml` in the repo root. This file is gitignored (machine-local settings), which creates two gotchas:
...
```
In the skill body: if the pre-resolved line is an absolute path, use it as `<repo-root>`; if it is empty or still shows a backtick command string, resolve `<repo-root>` at runtime by running `git rev-parse --show-toplevel` with the shell tool, then read `<repo-root>/.super-looper/config.local.yaml` with the `Read` tool. The runtime fallback is load-bearing: if `!` pre-resolution did not resolve, the agent must derive the root itself — otherwise config is silently ignored. Only when the root cannot be resolved (not a git repo) or the file is missing is it "no config" → fall through to defaults; never fail or block on missing config. (This mirrors the `sl-sessions` pre-resolution fallback.)
...
- `` !`cat "$(git rev-parse --show-toplevel 2>/dev/null)/.super-looper/config.local.yaml" 2>/dev/null || echo '__NO_CONFIG__'` `` aborts skill load on environments whose permission checker rejects `$()` command substitution (reported on Windows / CC 2.1.63 in issue #920).
...
When adding a new plugin alongside `super-looper`, register it with the marketplace and release tooling. Checklist:
...
- [ ] `.github/release-please-config.json` — add a `plugins/<name>` package entry with `extra-files` for the plugin.json path
...
The validator enforces name/version/description parity between the plugin's `plugin.json` and its `marketplace.json` entry. Note that only `description` drift is auto-corrected on `write: true` — version drift is detect-only because release-please owns the write.
