...
Use the workspace where the feature was built. Do not reinstall from scratch. If setup requires credentials or services, use `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to asking in chat only if it is unavailable or the call errors. Never silently skip the question.
...
Present the available tiers to the user via `AskUserQuestion` (call `ToolSearch` with `select:AskUserQuestion` first if its schema isn't loaded). Fall back to numbered options in chat only if it is unavailable or the call errors. Never silently skip the question. Mark the recommended tier. Always include "No evidence needed" as a final option.
...
Return these values to the caller (e.g., sl-commit-push-pr):
...
- The caller applies the label when formatting. sl-demo-reel does not generate markdown.
