...
| What it produces | A `optimize/<spec-name>` git branch with kept experiments merged in, plus an experiment log and strategy digest in `.context/super-looper/sl-optimize/<spec-name>/` |
| What's next | `/sl-code-review` on the cumulative diff; `/sl-compound` to capture the winning strategy; create a PR |
...
`sl-optimize` runs experimentation as a structured loop with explicit gates:
...
Rather than a single metric, `sl-optimize` evaluates each experiment in three layers:
...
You invoke `/sl-optimize "clustering quality on notification categorization"`. The skill detects this is qualitative — recommends `type: judge` because hard metrics like cluster count would optimize a misleading proxy. Walks you through defining stratified sampling (top by size, mid range, small clusters, plus singletons), the rubric (1-5 with concrete level descriptions), and the gates (`solo_pct <= 0.95`, `max_cluster_size <= 500`). Recommends serial mode and 4-iteration cap for the first run.

Phase 1 runs the measurement harness on baseline, dispatches `sl-learnings-researcher` for prior optimization context, runs the parallelism probe, and asks for explicit approval given the judge cost estimate. You approve.
...
After 4 iterations, the judge score has improved by 1.2 points and three experiments combined into the kept branch. Phase 4 surfaces post-completion options: code review, capture the winning strategy via `/sl-compound`, or create a PR.

---

## When to Reach For It

Reach for `sl-optimize` when:
...
`sl-optimize` is its own loop, but it interlocks with the chain:
...
- **Calls `sl-learnings-researcher`** during Phase 0.3 to consult `docs/solutions/` for prior optimization work on similar topics
- **Hands off to `/sl-code-review`** at Phase 4.3 — the cumulative diff (baseline to final) gets reviewed before merging
- **Hands off to `/sl-compound`** to document the winning strategy as institutional learning
...
`sl-optimize` is most often standalone — long-running optimization is its own activity:

- **From a spec** — `/sl-optimize path/to/spec.yaml` (use `references/example-hard-spec.yaml` or `references/example-judge-spec.yaml` as starting points)
- **From a description** — `/sl-optimize "reduce build time by 30%"` walks you through writing the spec interactively
- **Resume an existing run** — `/sl-optimize <spec-name>` detects the existing branch and offers Resume vs Fresh Start
...
The optimization branch (`optimize/<spec-name>`) with all kept-experiment commits is preserved. The experiment log and strategy digest stay in `.context/super-looper/sl-optimize/<spec-name>/` for local resume and audit (`.context/` is gitignored, so they don't travel with the branch).

---

## See Also

- [`sl-code-review`](./sl-code-review.md) — Phase 4.3 hand-off for reviewing the cumulative diff
- [`sl-compound`](./sl-compound.md) — capture the winning strategy as institutional learning
- [`sl-worktree`](./sl-worktree.md) — manual worktree creation if you want to set up isolation outside the optimize loop
