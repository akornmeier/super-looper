...
**Headless mode** changes the interaction model, not the classification boundaries. sl-doc-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
```
Skill("sl-doc-review", "mode:headless docs/plans/my-plan.md")
```
...
**If no document is specified (interactive mode):** Ask which document to review, or find the most recent in `docs/brainstorms/` or `docs/plans/` using the Glob tool.
...
```
Reviewing with:
- sl-coherence-reviewer (always-on)
- sl-feasibility-reviewer (always-on)
- sl-scope-guardian-reviewer -- plan has 12 requirements across 3 priority levels
- sl-security-lens-reviewer -- plan adds API endpoints with auth flow
```
...
Cross-session persistence is out of scope. A new invocation of sl-doc-review on the same document starts with a fresh round 1 and no carried primer, even if prior sessions deferred findings into the document's Open Questions section.
