...
| 2 | `TERM_PROGRAM=vscode` AND no Claude Code signal | Plain VS Code | Print the URL with a hint: `Open in VS Code: Ctrl+Shift+P → "Simple Browser: Show" → paste URL`. |
...
Hosts without an embedded-browser handoff fall through to the terminal branch (print the URL). When a convention emerges, add a new row to the detection table above.
