...
`sl-resolve-pr-feedback` runs feedback resolution as a structured pipeline:
...
Bot wrappers from review bots like CodeRabbit are dropped silently — recognized by boilerplate content, never announced or counted. This is a *content* check (is there anything actionable here?), not a source check, so it holds regardless of which bot's format changes.
...
A reviewer (and a review bot) leave 8 comments on your PR. You invoke `/sl-resolve-pr-feedback`.
...
Step 4 dispatches 6 `sl-pr-comment-resolver` agents in batches of 4. File-collision check: two threads touch `app/services/dispatcher.rb` → those two serialize; the rest run in parallel. Each agent reads the actual code and judges its finding on the merits:
...
Reach for `sl-resolve-pr-feedback` when:
...
Skip `sl-resolve-pr-feedback` when:

- The PR has no feedback yet
- You only want to ack the feedback without fixing — the skill expects to act, not just acknowledge
- The feedback is on a brainstorm or plan doc, not code → use `/sl-doc-review`

---

## Use as Part of the Workflow

`sl-resolve-pr-feedback` is the closing loop after `/sl-commit-push-pr` opens a PR:

```text
/sl-work → /sl-commit-push-pr → reviewer leaves comments → /sl-resolve-pr-feedback
```

It complements:

- **`/sl-code-review`** — reviews *before* the PR is open; this skill handles incoming feedback *after*
- **`/sl-debug`** — for broken behavior; this skill is for review-comment resolution
...
- **Current branch's PR** — `/sl-resolve-pr-feedback`
- **Specific PR** — `/sl-resolve-pr-feedback 1234`
- **Targeted (single thread)** — `/sl-resolve-pr-feedback https://github.com/.../pull/1234#discussion_r5678901`
...
- [`sl-code-review`](./sl-code-review.md) — pre-PR review; this skill handles post-PR feedback
- [`sl-commit-push-pr`](./sl-commit-push-pr.md) — opens the PR that this skill responds to
- [`sl-debug`](./sl-debug.md) — for broken behavior reported as a bug, not review feedback
- [`sl-doc-review`](./sl-doc-review.md) — for feedback on requirements or plan docs, not code
