#!/usr/bin/env python3
"""Extract session metadata from Claude Code JSONL files.
...
if files:
    ...
    for filepath in files:
        ...
        processed += 1
        if result:
            # Apply CWD filter first: cheap metadata-only check, applied before
            # paying the full-file keyword scan cost so files that are
            # immediately discarded are never scanned.
