...
> Check whether the installed super-looper plugin is up to date and recommend the update command if not. Claude Code only.

`sl-update` is the **plugin version check** skill. It compares the version your Claude Code session has loaded against `plugin.json` on `main` (where the marketplace installs from) and tells you whether you're up to date — and if not, gives you the exact `claude plugin update` command to run.

It's Claude Code only because it relies on the plugin harness cache layout (`~/.claude/plugins/cache/<marketplace>/super-looper/<version>/...`) for version detection. On other platforms or under `claude --plugin-dir` local development, the skill recognizes the case and tells you no action is needed.
...
| When to use it | "Update super looper", "is ce up to date", or when bug behavior suggests a stale plugin |
...
`sl-update` runs a focused version probe:
...
The marketplace installs plugin contents from `main` HEAD, not from the latest release tag. Comparing against tags would false-positive whenever `main` is ahead of the last tag — which is the normal state between releases. The skill reads `plugins/super-looper/.claude-plugin/plugin.json` on `main` directly via `gh api`.

### 2. Marketplace name detected from skill path, not hardcoded

The plugin distributes under multiple marketplace names — `super-looper-plugin` for public installs (per the README), and other names for internal or team marketplaces. Hardcoding a name into the update command would be wrong for half the audience. Instead, the skill parses `${CLAUDE_SKILL_DIR}` against the marketplace-cache layout (`~/.claude/plugins/cache/<marketplace>/super-looper/<version>/skills/sl-update`) and extracts the marketplace name from the path. The recommended `claude plugin update super-looper@<marketplace>` command uses the detected name.
...
The skill tells the user: "Loaded from outside the marketplace cache. Normal when using `claude --plugin-dir` for local development. No action for this session. Your marketplace install (if any) is unaffected — run `/sl-update` in a regular Claude Code session (no `--plugin-dir`) to check that cache."
...
`disable-model-invocation: true` prevents the skill from auto-firing on prose mentions of "update" or version-related discussion. Plugin updates are a deliberate user choice — invoke `/sl-update` directly.

---

## Quick Example

You suspect a skill behavior doesn't match what the docs say. You wonder if your plugin is stale. You invoke `/sl-update`.
...
- `marketplace-name.sh` returns `super-looper-plugin` (parsed from the same path)

Currently-loaded ≠ upstream → out of date. The skill responds:

```text
super-looper is on v2.40.0 but v2.42.0 is available.

Update with:
  claude plugin update super-looper@super-looper-plugin
...
Reach for `sl-update` when:

- You said "update super looper", "ce update", "is ce up to date"
...
`sl-update` is a standalone utility — it doesn't sit inside the chain. It's invoked when version drift is suspected:

- **From `/sl-report-bug`** — checking version is the first thing a bug report should establish
...
- `/sl-update` — runs the version check
...
- `upstream-version.sh` — `gh api` against `plugins/super-looper/.claude-plugin/plugin.json` on `main`
...
Because the plugin distributes under multiple marketplace names — public installs use `super-looper-plugin`, internal/team marketplaces use other names. Hardcoding the name into the update command would be wrong for half the audience. Detecting it from the cache path keeps the recommendation correct for whichever marketplace you're on.

**Why is it Claude Code only?**
Because version detection relies on the Claude Code plugin harness cache layout (`~/.claude/plugins/cache/<marketplace>/super-looper/<version>/...`). Other platforms have their own update mechanisms; this skill defers to those. When the skill detects it's not in the standard cache, it says so cleanly and stops.
...
- [`sl-setup`](./sl-setup.md) — installs missing dependencies; complementary to version checks
- [`sl-report-bug`](./sl-report-bug.md) — reporting a bug; should establish version first
- [`sl-release-notes`](./sl-release-notes.md) — summarize recent super-looper plugin releases
