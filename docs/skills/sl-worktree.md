...
| When to use it | Starting work that should stay isolated; when `sl-work` or `sl-code-review` offers a worktree option |
...
- **Phantom state** — a behind-the-back `git worktree add` is invisible to the harness (Orca, etc.) that owns worktree lifecycle: it can't list, navigate to, or clean it up.
...
`sl-worktree` runs isolation as an ordered decision, not a creation script:
...
When `sl-work` or `sl-code-review` invoke the skill, they pass a meaningful branch name derived from the work (`feat/crowd-sniff`, `fix/email-validation`) — never an opaque auto-generated name.

---

## Quick Example

You're in an Orca-managed worktree (the harness created it at session start) and ask `sl-work` to isolate the work. The skill runs Step 0: `--git-dir` and `--git-common-dir` differ, and the submodule guard returns empty → **you are already isolated**. It reports the worktree path and current branch and proceeds in place — no second worktree, no phantom state.
...
Reach for `sl-worktree` when:

- You're starting work that should stay isolated from the current checkout
- A skill (`sl-work`, `sl-code-review`) offered worktree as an option
...
`sl-worktree` is invoked from chain skills as their isolation step:

- **`/sl-work`** — when starting work, the user can choose worktree isolation over branching in the current checkout
- **`/sl-code-review`** — for reviewing PRs concurrently without disturbing in-progress work
...
`cd "$(git rev-parse --show-toplevel)"` to leave it, then `git worktree remove .worktrees/<branch>`. `/sl-clean-gone-branches` handles worktree-and-branch cleanup together when the remote tracking branch is gone.

---

## See Also

- [`/sl-work`](./sl-work.md) — offers this skill as its isolation option
- [`/sl-code-review`](./sl-code-review.md) — offers worktree isolation for concurrent review
- [`/sl-clean-gone-branches`](./sl-clean-gone-branches.md) — cleans up worktrees and branches together when the remote tracking branch is gone
