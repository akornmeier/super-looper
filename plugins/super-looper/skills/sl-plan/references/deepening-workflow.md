...
- `sl-spec-flow-analyzer` for missing user flows, edge cases, and handoff gaps
- `sl-repo-research-analyst` (Scope: `architecture, patterns`) for repo-grounded patterns, conventions, and implementation reality checks

**Context & Research / Sources & References gaps**
- `sl-learnings-researcher` for institutional knowledge and past solved problems
- `sl-framework-docs-researcher` for official framework or library behavior
- `sl-best-practices-researcher` for current external patterns and industry guidance
- `sl-web-researcher` for landscape/prior-art gaps — competitor patterns, market signals, or an unsettled external option set (which library/provider/approach) that recommendations depend on
- Add `sl-git-history-analyzer` only when historical rationale or prior art is materially missing

**Key Technical Decisions**
- `sl-architecture-strategist` for design integrity, boundaries, and architectural tradeoffs
- Add `sl-framework-docs-researcher` or `sl-best-practices-researcher` when the decision needs external grounding beyond repo evidence

**High-Level Technical Design**
- `sl-architecture-strategist` for validating that the technical design accurately represents the intended approach and identifying gaps
- `sl-repo-research-analyst` (Scope: `architecture, patterns`) for grounding the technical design in existing repo patterns and conventions
- Add `sl-best-practices-researcher` when the technical design involves a DSL, API surface, or pattern that benefits from external validation

**Implementation Units / Verification**
- `sl-repo-research-analyst` (Scope: `patterns`) for concrete file targets, patterns to follow, and repo-specific sequencing clues
- `sl-pattern-recognition-specialist` for consistency, duplication risks, and alignment with existing patterns
- Add `sl-spec-flow-analyzer` when sequencing depends on user flow or handoff completeness

**System-Wide Impact**
- `sl-architecture-strategist` for cross-boundary effects, interface surfaces, and architectural knock-on impact
- Add the specific specialist that matches the risk:
  - `sl-performance-oracle` for scalability, latency, throughput, and resource-risk analysis
  - `sl-security-sentinel` for auth, validation, exploit surfaces, and security boundary review
  - `sl-data-integrity-guardian` for migrations, persistent state safety, consistency, and data lifecycle risks

**Risks & Dependencies / Operational Notes**
- Use the specialist that matches the actual risk:
  - `sl-security-sentinel` for security, auth, privacy, and exploit risk
  - `sl-data-integrity-guardian` for migrations, backfills, persistent data safety, constraints, transaction boundaries, and production data transformation risk (plan context — not the PR-review `sl-data-migration-reviewer` persona)
  - `sl-deployment-verification-agent` for rollout checklists, rollback planning, and launch verification
  - `sl-performance-oracle` for capacity, latency, and scaling concerns
...
```bash
SCRATCH_DIR="$(mktemp -d -t sl-plan-deepen-XXXXXX)"
echo "$SCRATCH_DIR"
```
...
1. **Summarize the agent and its target section** — e.g., "The sl-architecture-strategist reviewed Key Technical Decisions and found:"
...
- Reorder or split implementation units when sequencing is weak — but **never renumber existing U-IDs**. Reordering preserves U-IDs in their new order (e.g., U1, U3, U5 reordered is correct; renumbering to U1, U2, U3 is not). Splitting keeps the original U-ID on the original concept and assigns the next unused number to the new unit. Renumbering breaks sl-work blocker and verification references that were written against the original IDs
...
- Recommend `sl-brainstorm` if the gap is truly product-defining
