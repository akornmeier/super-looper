...
const PLUGIN_ROOT = path.join(process.cwd(), "plugins", "super-looper", "skills")

/** Canonical copies live in sl-compound; mirrors must stay identical. */
...
const SKILLS_WITH_COPIES = ["sl-compound", "sl-compound-refresh"]

describe("sl-compound support file drift", () => {
...
// byte-duplicated across sl-plan, sl-brainstorm, and sl-ideate. There is no
// cross-skill shared-file mechanism (see plugins/super-looper/AGENTS.md
...
// them.
const RENDERING_SKILLS = ["sl-plan", "sl-brainstorm", "sl-ideate"]
const RENDERING_REFS = [
  "references/markdown-rendering.md",
  "references/html-rendering.md",
]

describe("format-rendering ref drift across sl-plan and sl-brainstorm", () => {
 ...
 * See issue #606.
 */
describe("sl-compound YAML safety rule presence", () => {
  ...
  test("sl-compound/SKILL.md points at YAML Safety Rules in both frontmatter-writing spots", async () => {
    const raw = await readFile(
      path.join(PLUGIN_ROOT, "sl-compound", "SKILL.md"),
      "utf8",
    )
  ...
  test("sl-compound-refresh per-action-flows reference points at YAML-safety rules in the Replace flow", async () => {
    ...
    const raw = await readFile(
      path.join(PLUGIN_ROOT, "sl-compound-refresh", "references", "per-action-flows.md"),
      "utf8",
    )
