...
`sl-frontend-design` is the **design-quality** skill. AI tends toward generic SaaS aesthetics — purple-on-white, Inter font, card-grid hero, decorative gradients, copy that sounds like prompt-language leaking into the UI. This skill counteracts that with explicit defaults that the user (or an existing design system) can override, structured pre-build planning that forces a visual thesis, and visual verification before declaring done. It works for greenfield builds and modifications to existing apps; it auto-detects an existing design system and yields to it when one is present.
...
`sl-frontend-design` runs frontend work as a structured pass with explicit defaults and verification:
...
3. **`agent-browser` CLI** — fallback; if not installed, says "run `/sl-setup`"
...
One iteration. Take a screenshot, assess against the litmus checks, fix glaring issues, move on. For multi-round iterative refinement, the `sl-design-iterator` agent handles that.
...
You're building a landing page for a new product, no existing repo conventions. You invoke `/sl-frontend-design "build a landing page for a notion-style note-taking app"`.
...
Reach for `sl-frontend-design` when:
...
Skip `sl-frontend-design` when:

- The work is non-frontend (API, backend, scripts) — wrong scope
- You have a fixed Figma spec and want exact translation — `sl-design-iterator` or a design-sync agent fits better
...
`sl-frontend-design` is mostly invoked directly when frontend work begins, but interlocks with:
...
- **Greenfield landing** — `/sl-frontend-design "build a landing page for X"`
- **App / dashboard** — `/sl-frontend-design "build a settings dashboard"`
- **Component** — `/sl-frontend-design "build a NotificationToggle component"`
- **Modification** — `/sl-frontend-design "redesign the pricing page"` (existing app; auto-detects)
...
- [`/sl-work`](./sl-work.md) — invokes this skill during frontend implementation
- [`/sl-polish`](./sl-polish.md) — late-stage UX refinement after the feature is functional
- [`/sl-test-browser`](./sl-test-browser.md) — verifies the implementation works after the design pass
- [`/sl-demo-reel`](./sl-demo-reel.md) — captures visual evidence for PR descriptions
