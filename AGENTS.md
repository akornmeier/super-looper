...
- **Merge policy:** All changes to `main` go through pull requests. The `main` ruleset enforces this: it requires a pull request and four status checks — `test`, `pr-title`, `pr-size`, and `pr-reviewed` — and it blocks branch deletion, force-pushes, and re-creation of the branch. A direct push to `main` is refused. The direct path would skip `release:validate`, the test suite, and PR title validation; past direct merges caused version drift requiring multi-PR recovery (see `docs/solutions/workflow/release-please-version-drift-recovery.md`). A bypass actor still exempts the maintainer from these rules and is scheduled for removal; treat the direct path as forbidden regardless. The required-checks list, the rollback path, and the stuck-gate detector are recorded in `docs/solutions/workflow/main-ruleset-contract.md` — read it before renaming a workflow job, since a required context is matched by literal name and a rename blocks every pull request.
