...
name: sl-update
description: |
  Check if the super-looper plugin is up to date and recommend the
  update command if not. Use when the user says "update super looper",
  "check super looper version", "ce update", "is super looper
  up to date", "update ce plugin", or reports issues that might stem from a
  stale super-looper plugin version. This skill only works in Claude
...
Verify the installed super-looper plugin version matches the upstream
`plugin.json` on `main`, and recommend the update command if it doesn't.
Claude Code only.

The upstream version comes from `plugins/super-looper/.claude-plugin/plugin.json`
...
`~/.claude/plugins/cache/<marketplace>/super-looper/<version>/skills/sl-update`.
...
> Your marketplace install (if any) is unaffected — run `/sl-update` in a
...
> "super-looper **v{version}** is installed and up to date."

**Out of date** — `currently_loaded != upstream`:

> "super-looper is on **v{currently_loaded}** but **v{upstream}** is available.
>
> Update with:
> ```
> claude plugin update super-looper@{marketplace_name}
> ```
...
multiple marketplace names (for example, `super-looper-plugin` for
