...
- **Merge policy:** All changes to `main` go through pull requests. Today this is a convention the ruleset does not yet enforce: the `main` ruleset blocks branch deletion, force-pushes, and re-creation of the branch, and auto-requests a Copilot review — but it requires neither a pull request nor any status check, and a bypass actor exempts the maintainer from even those rules. A direct push to `main` therefore succeeds, and it skips `release:validate`, the test suite, and PR title validation. Past direct merges have caused version drift requiring multi-PR recovery (see `docs/solutions/workflow/release-please-version-drift-recovery.md`), which is why the convention holds regardless of what the ruleset permits. Planned work adds a pull-request rule and a required-status-checks rule (`test`, `pr-title`, `pr-size`, `pr-reviewed`) and then removes the bypass actor; until that lands, treat the direct path as forbidden even though nothing refuses it.
