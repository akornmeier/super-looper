...
const PREFIX = "sl-"
const REF = `plugins/super-looper/AGENTS.md "Naming Convention"`

// Exemptions from the sl- prefix rule. Add entries here only with a written
// reason — the exemption list shouldn't become a silent junk drawer.
const SKILL_EXEMPTIONS = new Set<string>([
  // lfg ships as the public command `/lfg` (see plugins/super-looper/README.md).
  "lfg",
])
...
describe("super-looper skill sl- prefix", () => {
  ...
  for (const dirName of skillDirs) {
    ...
    test(`skill "${dirName}" frontmatter name uses sl- prefix`, () => {
...
describe("super-looper agent sl- prefix", () => {
  ...
  for (const fileName of agentFiles) {
    test(`agent file "${fileName}" uses sl- prefix`, () => {
      expect(
    ...
    test(`agent "${fileName}" frontmatter name uses sl- prefix`, () => {
