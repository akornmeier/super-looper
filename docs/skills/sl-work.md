...
`sl-work` is primarily software-focused — it commits, runs tests, opens PRs, and integrates with code review skills. It also has a lightweight **non-code carve-out**: a plan marked `execution: knowledge-work` (produced by `sl-plan`'s approach-altitude flow) routes to a knowledge-work path that reads sources, synthesizes, and produces a deliverable, skipping the code lifecycle. Other non-software work without that marker still effectively ends at `sl-plan`, and a human executes it.
...
| When to use it | Implementing a `sl-plan` plan; small/medium bare-prompt work; resuming partly-shipped work |
| What it produces | Commits + a PR (or just commits, no-PR path) |
| What's next | Review the PR; run `/sl-compound` to capture learnings |
...
`sl-work` runs execution as a structured process with explicit gates:
...
A plan with four implementation units arrives. `sl-work` reads it, picks up an `Execution note: test-first` on one unit, and notes a deferred-implementation question to keep in mind. It builds a task list with U-ID prefixes and confirms the current branch name is meaningful.
...
- Product behavior isn't decided yet → `/sl-brainstorm`
- Implementation guardrails aren't established for non-trivial work → `/sl-plan`
- The bug has a known root cause and an obvious fix → `/sl-debug`
...
/sl-work
   ...
   v
/sl-code-review     (optional escalation; auto-invoked at Tier 2)
   |
   v
/sl-compound        — capture the learning
```

After shipping, `/sl-compound` captures any reusable learning (bugs encountered, patterns established, conventions adopted) into `docs/solutions/` so future runs of `sl-plan` and `sl-work` benefit from the institutional memory.

---

## Use Standalone

Many people reach for `sl-work` directly with a bare prompt — `sl-plan` is overkill when scope is small and the agent can scope it itself.
...
For large bare-prompt scope (cross-cutting, sensitive surfaces, many files), `sl-work` recommends `/sl-brainstorm` or `/sl-plan` first — but proceeds with your choice.
...
**Why doesn't `sl-work` just write all the code from the plan's exact signatures?**
Because the plan deliberately doesn't have exact signatures — it has decisions, units, files, scope, and test scenarios. The plan is the WHAT; `sl-work` is the HOW. This separation keeps plans portable across weeks of code change and across implementer.
...
Resuming after context compaction, picking up someone else's branch, or returning to a partly-shipped plan are all common. Idempotency ensures `sl-work` doesn't silently reimplement what's already there.

**What's the Residual Work Gate?**
When a deeper code review tier surfaces things the autofix didn't resolve, `sl-work` won't silently ship them. It asks: apply now / file tickets / accept (with durable sink) / stop. "Accept" requires a real durable record — findings can't live only in the session.

**Does `sl-work` support non-software plans?**
For a plan marked `execution: knowledge-work` (produced by `sl-plan`'s approach-altitude flow), yes — a lightweight carve-out reads the sources, synthesizes, and produces the deliverable, skipping the commit/test/PR lifecycle. Other non-software work without that marker still ends at `sl-plan`, and a human executes it.

---

## See Also

- [`sl-plan`](./sl-plan.md) — produces the guardrails `sl-work` executes against
- [`sl-brainstorm`](./sl-brainstorm.md) — defines what the plan should accomplish
- [`sl-ideate`](./sl-ideate.md) — upstream "what's worth exploring" discovery
- [`sl-code-review`](./sl-code-review.md) — Tier 2 escalation target
- [`sl-commit-push-pr`](./sl-commit-push-pr.md) — handles the final commit + PR flow
- [`sl-compound`](./sl-compound.md) — capture reusable learning after shipping
