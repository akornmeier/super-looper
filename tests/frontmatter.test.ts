  ...
  for (const pluginRoot of pluginRoots) {
    ...
    for (const [rel, yaml] of files) {
      ...
      if (/^skills\/[^/]+\/SKILL\.md$/.test(rel)) {
        ...
        // `name: sl:brainstorm` frontmatter in a renamed `sl-brainstorm` directory
        ...
        // All super-looper skills (and agents) must use the `sl-` prefix
        // so they are unambiguously identifiable as super-looper
        // components. See plugins/super-looper/AGENTS.md "Naming
        // Convention". A small allowlist preserves three pre-existing skills
        // that predate the rule -- no new entries should be added.
        if (pluginRoot === "plugins/super-looper") {
          const SKILL_PREFIX_ALLOWLIST = new Set([
            "every-style-editor",
            "file-todos",
            "lfg",
          ])
          test(`${pluginRoot}/${rel} skill name uses sl- prefix`, () => {
            const dirName = path.basename(path.dirname(rel))
            if (SKILL_PREFIX_ALLOWLIST.has(dirName)) return
            expect(
              dirName.startsWith("sl-"),
              `Skill "${dirName}" must use the sl- prefix. ` +
      ...
      if (
        pluginRoot === "plugins/super-looper" &&
        /^agents\/[^/]+\.md$/.test(rel)
      ) {
        test(`${pluginRoot}/${rel} agent name uses sl- prefix`, () => {
          const fileName = path.basename(rel, ".md")
          expect(
            fileName.startsWith("sl-"),
            `Agent "${fileName}" must use the sl- prefix.`,
          ).toBe(true)
        })
        ...
        // issue #832 for the coherence-reviewer stall).
