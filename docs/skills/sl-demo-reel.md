...
`sl-demo-reel` is the **evidence capture** skill. It detects the project type, recommends the right capture tier (browser reel / terminal recording / screenshot reel / static screenshots), records the actual feature in action, uploads to a public URL, and returns markdown ready for a PR description. **Evidence means using the product**, not running tests — "I ran npm test" is test evidence. Capture is running the actual CLI command, opening the web app, making the API call, or triggering the feature.

It's most often invoked by `/sl-commit-push-pr` when a change has observable behavior, but also directly when you want to add a demo to a PR description after the fact.
...
| What it produces | A public URL (or local path) and a `Tier`/`Description` ready for `sl-commit-push-pr` to splice into a PR body |
...
`sl-demo-reel` runs as a structured capture flow with explicit fallbacks:
...
The skill returns a structured envelope (`Tier`, `Description`, `URL`, `Path`) where exactly one of `URL` or `Path` carries a real value (the other is `"none"`). The caller — usually `/sl-commit-push-pr` — formats this into the PR description's `## Demo` or `## Screenshots` section. Static screenshots get the "Screenshots" label; all motion tiers get "Demo." Test output never gets either.

---

## Quick Example

You finish a notification settings page. You invoke `/sl-commit-push-pr`, which detects observable behavior and asks whether to capture evidence. You say yes. It loads `/sl-demo-reel`.
...
`/sl-commit-push-pr` splices `## Demo` with the GIF embed into the PR body. Total elapsed time: ~30 seconds.

---

## When to Reach For It

Reach for `sl-demo-reel` when:
...
`sl-demo-reel` is invoked by other skills when behavior is observable:

- **`/sl-commit-push-pr` Step 6** — calls this skill when the change has UI / CLI / API behavior and asks the user whether to capture
- **`/sl-work` Phase 4.1** — Evidence Context — flags whether evidence is possible so `sl-commit-push-pr` can ask the right question
...
- **After a PR is already open** — `/sl-demo-reel "the new settings page"` to add a demo to an existing PR
- **For a specific behavior** — `/sl-demo-reel "CLI output of the migrate command"`
- **Without a description** — `/sl-demo-reel` infers from branch/PR/diff context and asks if ambiguous

When invoked outside `sl-commit-push-pr`, the user typically copies the returned markdown into the PR description manually, or uses `gh pr edit --body-file` separately.
...
- [`sl-commit-push-pr`](./sl-commit-push-pr.md) — primary caller; splices the captured evidence into PR descriptions
- [`sl-work`](./sl-work.md) — flags evidence context at Phase 4.1 so the PR flow can ask the right question
- [`sl-test-browser`](./sl-test-browser.md) — sibling skill for end-to-end browser testing (different goal: verify behavior, not capture)
