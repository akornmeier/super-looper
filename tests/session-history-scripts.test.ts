...
const SCRIPTS_DIR = path.join(
  __dirname,
  "../plugins/super-looper/skills/sl-sessions/scripts"
)
...
describe("extract-metadata", () => {
  ...
  describe("--keyword mode", () => {
    test("filters to sessions matching a single keyword", async () => {
      ...
      // The Claude fixture mentions middleware.
...
// This lets sl-sessions route bulk extraction content to a scratch file
...
describe("--output PATH mode", () => {
  function tmpFile(): string {
    return path.join(
      fs.mkdtempSync(path.join(os.tmpdir(), "sl-sessions-test-")),
      "out.txt"
    )
  }
...
describe("platform-detection", () => {
  test("scripts detect the Claude platform", async () => {
    const fixturePath = path.join(FIXTURES_DIR, "claude-session.jsonl")
