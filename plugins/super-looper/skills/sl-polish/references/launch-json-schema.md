...
Polish reads `.claude/launch.json` at the repo root to resolve the dev-server start command. The schema is a subset of VS Code's `launch.json` format — chosen because Claude Code and VS Code both understand it and because users often already have one for editor integration.
...
`.claude/launch.json` is not yet a fully unified standard across Claude Code and VS Code. Polish leads with `.claude/launch.json` because:
- Claude Code and VS Code can both read it as a launch config
